...
public abstract class MesAnoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected MesAnoAgregadoI getItem() {
		return (MesAnoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (MesAno) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataMesAno);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para MesAnoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em MesAnoQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(MesAno valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(MesAno valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de MesAnoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(MesAno valor) {
		MesAnoServico exibicaoSrv = FabricaServico.getInstancia().getMesAnoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_mesano, null);
	...
	// outro quadro
	protected final void insereItem(MesAno valor) {
		MesAnoServico exibicaoSrv = FabricaServico.getInstancia().getMesAnoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_mesano, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(MesAno vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em MesAnoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final MesAno vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em MesAnoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_mesano,"R.layout.trata_mesano");
		return recurso;
	}
