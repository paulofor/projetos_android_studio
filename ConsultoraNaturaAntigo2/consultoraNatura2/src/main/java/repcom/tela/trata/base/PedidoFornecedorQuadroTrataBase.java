...
public abstract class PedidoFornecedorQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected PedidoFornecedorAgregadoI getItem() {
		return (PedidoFornecedorAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (PedidoFornecedor) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataPedidoFornecedor);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para PedidoFornecedorQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em PedidoFornecedorQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(PedidoFornecedor valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(PedidoFornecedor valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de PedidoFornecedorQuadroTrataBase");
	//}
	...
	protected final void alteraItem(PedidoFornecedor valor) {
		PedidoFornecedorServico exibicaoSrv = FabricaServico.getInstancia().getPedidoFornecedorServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_pedido_fornecedor, null);
	...
	// outro quadro
	protected final void insereItem(PedidoFornecedor valor) {
		PedidoFornecedorServico exibicaoSrv = FabricaServico.getInstancia().getPedidoFornecedorServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_pedido_fornecedor, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(PedidoFornecedor vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em PedidoFornecedorQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final PedidoFornecedor vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em PedidoFornecedorQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_pedido_fornecedor,"R.layout.trata_pedido_fornecedor");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaPedidoFornecedor();
