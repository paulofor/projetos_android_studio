...
import repcom.modelo.agregado.LinhaProdutoAgregadoI;
...
public abstract class LinhaProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected LinhaProdutoAgregadoI getItem() {
		return (LinhaProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (LinhaProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataLinhaProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para LinhaProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em LinhaProdutoQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(LinhaProduto valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(LinhaProduto valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de LinhaProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(LinhaProduto valor) {
		LinhaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getLinhaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_linha_produto, null);
	...
	// outro quadro
	protected final void insereItem(LinhaProduto valor) {
		LinhaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getLinhaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_linha_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(LinhaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em LinhaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final LinhaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em LinhaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_linha_produto,"R.layout.trata_linha_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaLinhaProduto();
