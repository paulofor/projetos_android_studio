...
public abstract class EstoqueQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected EstoqueAgregadoI getItem() {
		return (EstoqueAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (Estoque) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataEstoque);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para EstoqueQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em EstoqueQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(Estoque valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(Estoque valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de EstoqueQuadroTrataBase");
	//}
	...
	protected final void alteraItem(Estoque valor) {
		EstoqueServico exibicaoSrv = FabricaServico.getInstancia().getEstoqueServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_estoque, null);
	...
	// outro quadro
	protected final void insereItem(Estoque valor) {
		EstoqueServico exibicaoSrv = FabricaServico.getInstancia().getEstoqueServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_estoque, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(Estoque vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em EstoqueQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final Estoque vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em EstoqueQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_estoque,"R.layout.trata_estoque");
		return recurso;
	}
