...
public abstract class ContatoClienteQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected ContatoClienteAgregadoI getItem() {
		return (ContatoClienteAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (ContatoCliente) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataContatoCliente);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para ContatoClienteQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em ContatoClienteQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(ContatoCliente valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(ContatoCliente valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de ContatoClienteQuadroTrataBase");
	//}
	...
	protected final void alteraItem(ContatoCliente valor) {
		ContatoClienteServico exibicaoSrv = FabricaServico.getInstancia().getContatoClienteServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_contato_cliente, null);
	...
	// outro quadro
	protected final void insereItem(ContatoCliente valor) {
		ContatoClienteServico exibicaoSrv = FabricaServico.getInstancia().getContatoClienteServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_contato_cliente, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(ContatoCliente vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em ContatoClienteQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final ContatoCliente vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em ContatoClienteQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_contato_cliente,"R.layout.trata_contato_cliente");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			ContatoClienteServico servico = FabricaServico.getInstancia().getContatoClienteServico(FabricaServico.TIPO_SQLITE);
			item = FabricaVo.criaContatoCliente();
			item = servico.inicializaItemTela(item, getContext());
			
			if (this.cliente!=null) {
				item.setCliente_ReferenteA(cliente);
			}
