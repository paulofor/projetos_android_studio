...
public abstract class CategoriaProdutoProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected CategoriaProdutoProdutoAgregadoI getItem() {
		return (CategoriaProdutoProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (CategoriaProdutoProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataCategoriaProdutoProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para CategoriaProdutoProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em CategoriaProdutoProdutoQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(CategoriaProdutoProduto valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(CategoriaProdutoProduto valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de CategoriaProdutoProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(CategoriaProdutoProduto valor) {
		CategoriaProdutoProdutoServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaProdutoProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_produto_produto, null);
	...
	// outro quadro
	protected final void insereItem(CategoriaProdutoProduto valor) {
		CategoriaProdutoProdutoServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaProdutoProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_produto_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(CategoriaProdutoProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em CategoriaProdutoProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final CategoriaProdutoProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em CategoriaProdutoProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_categoria_produto_produto,"R.layout.trata_categoria_produto_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaCategoriaProdutoProduto();
