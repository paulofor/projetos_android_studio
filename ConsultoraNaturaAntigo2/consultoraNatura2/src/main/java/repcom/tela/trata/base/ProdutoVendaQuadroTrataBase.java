...
public abstract class ProdutoVendaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected ProdutoVendaAgregadoI getItem() {
		return (ProdutoVendaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (ProdutoVenda) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataProdutoVenda);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para ProdutoVendaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em ProdutoVendaQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(ProdutoVenda valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(ProdutoVenda valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de ProdutoVendaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(ProdutoVenda valor) {
		ProdutoVendaServico exibicaoSrv = FabricaServico.getInstancia().getProdutoVendaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_produto_venda, null);
	...
	// outro quadro
	protected final void insereItem(ProdutoVenda valor) {
		ProdutoVendaServico exibicaoSrv = FabricaServico.getInstancia().getProdutoVendaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_produto_venda, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(ProdutoVenda vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em ProdutoVendaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final ProdutoVenda vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em ProdutoVendaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_produto_venda,"R.layout.trata_produto_venda");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaProdutoVenda();
	...
	protected Spinner getSpinnerVenda(int codigo, String nome) {
		VendaServico srv = FabricaServico.getInstancia().getVendaServico(FabricaServico.TIPO_SQLITE);
		List<Venda> listaTela = srv.getAllTela(getContext().getContext());
		Spinner spn = this.getSpinner(codigo, nome, listaTela, this.getContext().getContext()); 
		return spn;
	}
	/*
	protected Spinner carregaSpinnerVenda(Spinner spn) {
		VendaServico srv = FabricaServico.getInstancia().getVendaServico(FabricaServico.TIPO_SQLITE);
		List<Venda> listaTela = srv.getAllTela(getContext().getContext());
		ArrayAdapter<Venda> dataAdapter = new ArrayAdapter<Venda>(getContext().getContext(), android.R.layout.simple_spinner_item, listaTela);
