...
public abstract class CategoriaProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected CategoriaProdutoAgregadoI getItem() {
		return (CategoriaProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (CategoriaProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataCategoriaProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para CategoriaProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em CategoriaProdutoQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(CategoriaProduto valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(CategoriaProduto valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de CategoriaProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(CategoriaProduto valor) {
		CategoriaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_produto, null);
	...
	// outro quadro
	protected final void insereItem(CategoriaProduto valor) {
		CategoriaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(CategoriaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em CategoriaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final CategoriaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em CategoriaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_categoria_produto,"R.layout.trata_categoria_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			CategoriaProdutoServico servico = FabricaServico.getInstancia().getCategoriaProdutoServico(FabricaServico.TIPO_SQLITE);
			item = FabricaVo.criaCategoriaProduto();
