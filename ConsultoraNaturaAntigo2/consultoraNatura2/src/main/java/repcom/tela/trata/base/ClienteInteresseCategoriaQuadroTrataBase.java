...
public abstract class ClienteInteresseCategoriaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected ClienteInteresseCategoriaAgregadoI getItem() {
		return (ClienteInteresseCategoriaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (ClienteInteresseCategoria) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataClienteInteresseCategoria);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para ClienteInteresseCategoriaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em ClienteInteresseCategoriaQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(ClienteInteresseCategoria valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(ClienteInteresseCategoria valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de ClienteInteresseCategoriaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(ClienteInteresseCategoria valor) {
		ClienteInteresseCategoriaServico exibicaoSrv = FabricaServico.getInstancia().getClienteInteresseCategoriaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_cliente_interesse_categoria, null);
	...
	// outro quadro
	protected final void insereItem(ClienteInteresseCategoria valor) {
		ClienteInteresseCategoriaServico exibicaoSrv = FabricaServico.getInstancia().getClienteInteresseCategoriaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_cliente_interesse_categoria, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(ClienteInteresseCategoria vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em ClienteInteresseCategoriaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final ClienteInteresseCategoria vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em ClienteInteresseCategoriaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_cliente_interesse_categoria,"R.layout.trata_cliente_interesse_categoria");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaClienteInteresseCategoria();
