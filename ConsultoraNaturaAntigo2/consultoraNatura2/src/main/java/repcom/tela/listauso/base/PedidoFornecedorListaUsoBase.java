...
import repcom.view.adapter.listauso.PedidoFornecedorListUsoAdapter;
...
public abstract class PedidoFornecedorListaUsoBase extends BaseListFragment implements IQuadroLista {

	public final static String ITEM_CLICK = "PedidoFornecedorItemClick";

	private PedidoFornecedorListUsoAdapterBase adapt = null;
	...
	protected void atualizaLista(List<PedidoFornecedor> listaNova) {
	...
	*/
	...
	private void preencheLista() {
        ...
        List<PedidoFornecedor> lista = getListaCorrente(dContexto.getContext(),servico);
        // Pode ser necessario um adapter customizado (diferenciar o editar e usar)
        adapt =  getAdapter(lista, dContexto);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "getListaCorrente : List<PedidoFornecedor> -> " + lista.size() + " itens (Adapter:" + adapt.getClass().getName() + ")");
        adapt.setRaiz(this);
        setListAdapter(adapt);
	}
	
	protected PedidoFornecedorListUsoAdapterBase getAdapter(List<PedidoFornecedor> lista,DigicomContexto dContexto) {
		return new PedidoFornecedorListUsoAdapter(lista,dContexto);
	}
