...
import repcom.view.adapter.listauso.ProdutoVendaListUsoAdapter;
...
public abstract class ProdutoVendaListaUsoBase extends BaseListFragment implements IQuadroLista {

	public final static String ITEM_CLICK = "ProdutoVendaItemClick";

	private ProdutoVendaListUsoAdapterBase adapt = null;
	...
	protected void atualizaLista(List<ProdutoVenda> listaNova) {
	...
	*/
	...
	private void preencheLista() {
        ...
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "getListaCorrente : List<ProdutoVenda> -> " + lista.size() + " itens (Adapter:" + adapt.getClass().getName() + ")");
        adapt.setRaiz(this);
        setListAdapter(adapt);
	}
	
	protected ProdutoVendaListUsoAdapterBase getAdapter(List<ProdutoVenda> lista,DigicomContexto dContexto) {
		return new ProdutoVendaListUsoAdapter(lista,dContexto);
	}
	
	protected List<ProdutoVenda> getListaCorrente(Context contexto,ProdutoVendaServico servico) {
		
		throw new UnsupportedOperationException("Fazer override de getListaCorrente em ProdutoVendaListaUso, trata-se de classe de relacionamento");
