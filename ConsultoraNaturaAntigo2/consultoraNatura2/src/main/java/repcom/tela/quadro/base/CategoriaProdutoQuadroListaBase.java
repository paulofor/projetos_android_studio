...
import repcom.view.adapter.CategoriaProdutoListAdapter;
...
public abstract class CategoriaProdutoQuadroListaBase extends BaseListFragment implements IQuadroListaEdicao {

	public final static String ITEM_CLICK = "CategoriaProdutoItemClick";

	CategoriaProdutoListAdapter adapt = null;
	private Button btnCriaItem = null;
	
	CategoriaProdutoServico servico = null;
	...
	protected void inicializaItensTelaBase() {
		btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaCategoriaProduto);
	}
	@Override
	...
	protected void inicializaServicos() {
		servico = FabricaServico.getInstancia().getCategoriaProdutoServico(FabricaServico.TIPO_SQLITE);
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_categoria_produto,"R.layout.lista_categoria_produto");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//CategoriaProduto nova = servico.inicializaItemTela(getContext());
		CategoriaProduto nova = FabricaVo.criaCategoriaProduto();
	...
	protected CategoriaProduto criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em CategoriaProdutoQuadroLista retornando new CategoriaProduto com inicializa??o de listas internas se necessario");
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em CategoriaProdutoQuadroLista retornando new CategoriaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private void preencheLista() {
        ...
        List<CategoriaProduto> lista = getListaCorrente(dContexto.getContext(),servico);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<CategoriaProduto> -> " + lista.size() + " itens");
	...
	protected CategoriaProdutoListAdapter getAdapter(List<CategoriaProduto> lista,DigicomContexto dContexto) {
		return new CategoriaProdutoListAdapter(lista,dContexto.getContext());
	}
	
	protected List<CategoriaProduto> getListaCorrente(Context contexto,CategoriaProdutoServico servico) {
		List<CategoriaProduto> saida = servico.getAllTela(contexto);
		return saida;
	}
