...
import repcom.view.adapter.ContatoClienteListAdapter;
...
public abstract class ContatoClienteQuadroListaBase extends BaseListFragment implements IQuadroListaEdicao {

	public final static String ITEM_CLICK = "ContatoClienteItemClick";

	ContatoClienteListAdapter adapt = null;
	private Button btnCriaItem = null;
	
	ContatoClienteServico servico = null;
	...
	protected void inicializaItensTelaBase() {
		btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaContatoCliente);
	}
	@Override
	...
	protected void inicializaServicos() {
		servico = FabricaServico.getInstancia().getContatoClienteServico(FabricaServico.TIPO_SQLITE);
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_contato_cliente,"R.layout.lista_contato_cliente");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//ContatoCliente nova = servico.inicializaItemTela(getContext());
		ContatoCliente nova = FabricaVo.criaContatoCliente();
	...
	protected ContatoCliente criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em ContatoClienteQuadroLista retornando new ContatoCliente com inicializa??o de listas internas se necessario");
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em ContatoClienteQuadroLista retornando new ContatoClienteQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private void preencheLista() {
        ...
        List<ContatoCliente> lista = getListaCorrente(dContexto.getContext(),servico);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<ContatoCliente> -> " + lista.size() + " itens");
	...
	protected ContatoClienteListAdapter getAdapter(List<ContatoCliente> lista,DigicomContexto dContexto) {
		return new ContatoClienteListAdapter(lista,dContexto.getContext());
	}
	
	protected List<ContatoCliente> getListaCorrente(Context contexto,ContatoClienteServico servico) {
		List<ContatoCliente> saida = servico.getAllTela(contexto);
		return saida;
	}
