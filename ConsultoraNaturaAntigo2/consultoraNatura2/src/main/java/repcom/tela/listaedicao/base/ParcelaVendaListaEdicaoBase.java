...
import repcom.view.adapter.ParcelaVendaListAdapter;
import repcom.view.adapter.listaedicao.ParcelaVendaListEdicaoAdapter;
...
public abstract class ParcelaVendaListaEdicaoBase extends BaseListFragment implements IQuadroListaEdicao  
	{

	public final static String ITEM_CLICK = "ParcelaVendaItemClick";

	private ParcelaVendaListEdicaoAdapter adapt = null;
	private List<ParcelaVenda> lista = null;
	private Button btnCriaItem = null;
	
	private boolean salvaPreliminar = false;
	
	private ParcelaVendaServico servico = null;
	...
	protected ParcelaVendaServico getServico() {
		return servico;
	}
	
	@Override
	protected final void inicializaItensTelaBase() {
		btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaParcelaVenda);
	}
	@Override
	...
	protected final void inicializaServicosBase() {
		servico = FabricaServico.getInstancia().getParcelaVendaServico(FabricaServico.TIPO_SQLITE);
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_parcela_venda,"R.layout.lista_parcela_venda");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//ParcelaVenda nova = servico.inicializaItemTela(getContext());
		ParcelaVenda nova = FabricaVo.criaParcelaVenda();
		nova = insereObjetoPrincipal(nova);
		nova = (ParcelaVenda) ((ServicoLocal)servico).atribuiUsuario(nova);
	...
	/*
	protected ParcelaVenda criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em ParcelaVendaQuadroLista retornando new ParcelaVenda com inicializa??o de listas internas se necessario");
	...
	*/
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em ParcelaVendaListaEdicao retornando new ParcelaVendaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private void preencheLista() {
        ...
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<ParcelaVenda> -> " + lista.size() + " itens");
	...
	private ParcelaVendaListEdicaoAdapter getAdapter(List<ParcelaVenda> lista,DigicomContexto dContexto) {
		return new ParcelaVendaListEdicaoAdapter(lista,this,dContexto.getContext());
	}
	...
	private ParcelaVenda insereObjetoPrincipal(ParcelaVenda item) {
	
		return item;
	}
	
	
	protected List<ParcelaVenda> getListaCorrente(Context contexto,ParcelaVendaServico servico) {
		List<ParcelaVenda> saida = servico.getAllTela(contexto);
		return saida;
	}
