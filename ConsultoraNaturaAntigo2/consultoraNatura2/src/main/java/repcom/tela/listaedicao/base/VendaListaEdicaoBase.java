...
import repcom.view.adapter.listaedicao.VendaListEdicaoAdapter;
...
public abstract class VendaListaEdicaoBase extends BaseListFragment implements IQuadroListaEdicao  
	{

	public final static String ITEM_CLICK = "VendaItemClick";

	private VendaListEdicaoAdapter adapt = null;
	private List<Venda> lista = null;
	private Button btnCriaItem = null;
	
	private boolean salvaPreliminar = false;
	
	private VendaServico servico = null;
	...
	protected VendaServico getServico() {
		return servico;
	}
	...
	public final void chamaCriaItem() {
		...
		nova = (Venda) ((ServicoLocal)servico).atribuiUsuario(nova);
	...
	*/
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em VendaListaEdicao retornando new VendaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private VendaListEdicaoAdapter getAdapter(List<Venda> lista,DigicomContexto dContexto) {
		return new VendaListEdicaoAdapter(lista,this,dContexto.getContext());
	}
	...
	private Venda insereObjetoPrincipal(Venda item) {
	...
	protected List<Venda> getListaCorrente(Context contexto, VendaServico servico) {
		...
		if (cliente!=null) {
			listaSaida = servico.getPorFeitaParaCliente(contexto, cliente.getId());
		}
