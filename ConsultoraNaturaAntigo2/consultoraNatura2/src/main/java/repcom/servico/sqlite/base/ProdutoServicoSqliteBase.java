...
public abstract class ProdutoServicoSqliteBase extends  ProdutoServico 
	...
	//public List<Produto> ListaPorIdCategoria(Context contexto){}
	//public List<Produto> PesquisaTrechoNome(Context contexto){}
	...
	public List<Produto> getPorEstaEmLinhaProduto(Context contexto, long id) {
		List<Produto> saida = null; 
		try {
			saida = getDao().getPorEstaEmLinhaProduto(contexto, id);
			DCLog.dStack(DCLog.SERVICO_OPERACAO_PADRAO, this, "getPorEstaEmLinhaProduto : " +  saida.size() + " itens" );
		} catch (DaoException e) {
			DCLog.e(DCLog.DATABASE_ERRO, this, e);
		}
		return saida;
	...
	public List<Produto> getPorEstaEmLinhaProduto(long id) {
		List<Produto> saida = null; 
		try {
			saida = getDao().getPorEstaEmLinhaProduto(id);
			DCLog.dStack(DCLog.SERVICO_OPERACAO_PADRAO, this, "getPorEstaEmLinhaProduto : " +  saida.size() + " itens" );
		} catch (DaoException e) {
			DCLog.e(DCLog.DATABASE_ERRO, this, e);
		}
	...
	public final List<Produto> ListaPorIdCategoria(DigicomContexto contexto ) {
		List<Produto> saida = ListaPorIdCategoriaImpl(contexto);
		DCLog.dStack(DCLog.SERVICO_OPERACAO, this, "ListaPorIdCategoria : " +  saida.size() + " itens" );
		return saida;
	}
	protected abstract List<Produto> ListaPorIdCategoriaImpl(DigicomContexto contexto );
	public final List<Produto> PesquisaTrechoNome(DigicomContexto contexto ) {
		List<Produto> saida = PesquisaTrechoNomeImpl(contexto);
		DCLog.dStack(DCLog.SERVICO_OPERACAO, this, "PesquisaTrechoNome : " +  saida.size() + " itens" );
		return saida;
	}
	protected abstract List<Produto> PesquisaTrechoNomeImpl(DigicomContexto contexto );
	@Override
