...
public abstract class ClienteServicoWsJsonBase 
		extends  ClienteServico 
		implements ServicoRemoto<Cliente>{

	private List<Cliente> listaSaida = null;
	...
	public ClienteServicoWsJsonBase() {
		setDataSource(new DataSourceRemoto());
	}
	
	
	@Override
	public void listaSincronizadaAlteracao(List<Cliente> listaSinc, Context contexto) throws JSONException {
		Map<String,String> params = new HashMap<String,String>();
		String url = getUrl("ListaSincronizadaAlteracao");
		String saida = " [ ";
		boolean primeiro = true;
		for (Cliente item:listaSinc) {
			if (primeiro) {
				primeiro = false;
			} else {
				saida += " , ";
		...
		DCLog.d(DCLog.SINCRONIZACAO_JSON,this,"Enviando...lista<Cliente>:" + saida);
	...
	private static final String URL_GET_POSTS_RECENT = 
		"http://www.lojadigicom.com.br/repcom/page/ClientePage.aspx";
	...
	private final Handler handler = new Handler() {
		public void handleMessage(final Message msg) {
			...
			listaSaida = new ArrayList<Cliente>();
			try {
				json = new JSONObject(bundleResult);
				JSONArray lista = json.getJSONArray("Cliente");
				int tam = lista.length();
				JSONObject item = null;
				String nome = null;
				for (int i=0;i<lista.length();i++) {
					item = (JSONObject) lista.get(i);
					listaSaida.add(new Cliente(item));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	*/
	...
	public List<Cliente> ObtemListaAgendaTel(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("ObtemListaAgendaTel"));
		return processaResultado(resultado);
	} 
	public List<Cliente> SincronizaAgendaTel(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("SincronizaAgendaTel"));
		return processaResultado(resultado);
	}
	public Cliente ObtemPorIdAgendaTel(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("ObtemPorIdAgendaTel"));
		return processaResultadoObjeto(resultado);
	}
	public Cliente PreencheDerivadaAgendaTel(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("PreencheDerivadaAgendaTel"));
		return processaResultadoObjeto(resultado);
	}
	public Cliente DesativaPorId(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("DesativaPorId"));
		return processaResultadoObjeto(resultado);
	} 
	public List<Cliente> ListaAtivos(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("ListaAtivos"));
		return processaResultado(resultado);
	} 
	public List<Cliente> PesquisaTrechoNome(DigicomContexto ctx){
	...
	private int processaResultado(String resultado, DaoSincronismo daoSinc) {
		...
		try {
			...
			for (int i=0;i<lista.length();i++) {
				item = (JSONObject) lista.get(i);
				obj = FabricaVo.criaCliente(item);
				daoSinc.insere(obj);
			}
			lista = null;
		} catch (Exception e) {
	...
	@Deprecated
	private List<Cliente> processaResultado(String resultado) {
		int inicio = resultado.indexOf('{');
		resultado = resultado.substring(inicio);
		List<Cliente> listaSaida = new LinkedList<Cliente>();
		...
		try {
			...
			for (int i=0;i<lista.length();i++) {
				item = (JSONObject) lista.get(i);
				//corrente = FabricaVo.criaCliente(item);
				//servicoLocal.insertLocal(corrente);
				listaSaida.add(FabricaVo.criaCliente(item));
			}
			lista = null;			
		} catch (Exception e) {
			//Log.e("ClienteServicoWsJson", e.getMessage() + " [ " + ultimaUrl + " ]");
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
		}
	...
	private Cliente processaResultadoObjeto(String resultado) {
		int inicio = resultado.indexOf('{');
		resultado = resultado.substring(inicio);
		Cliente saida = null;
		...
		try {
			...
			String nome = null;
			saida = FabricaVo.criaCliente(item);
	...
	public final void alteraParaSincronizacao(Cliente item) {
		throw new UnsupportedOperationException();
	}
	public final void insereParaSincronizacao(Cliente item) {
		throw new UnsupportedOperationException();
	}
	...
	public final Cliente ultimoInicializado() {
		throw new UnsupportedOperationException();
	}
	...
	*/
	...
	public List<Cliente> listaSincronizadaUsuario(Context contexto) {
	...
	@Deprecated
	public List<Cliente> listaSincronizadaUsuarioV2(Context contexto) {
	...
	private List<Cliente> processaResultadoLista(List<String> lista) {
		List<Cliente> listaSaida = new LinkedList<Cliente>();
		try {
			for (String palavra : lista) {
				JSONObject json = new JSONObject(palavra);
				Cliente item = FabricaVo.criaCliente(json);
				listaSaida.add(item);
			}
		} catch (Exception e) {
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
	...
	private int processaResultadoLista(List<String> lista , DaoSincronismo daoSinc) {
		...
		try {
			...
			for (String palavra : lista) {
				JSONObject json = new JSONObject(palavra);
				item = FabricaVo.criaCliente(json);
				daoSinc.insere(item);
			}
		} catch (Exception e) {
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
