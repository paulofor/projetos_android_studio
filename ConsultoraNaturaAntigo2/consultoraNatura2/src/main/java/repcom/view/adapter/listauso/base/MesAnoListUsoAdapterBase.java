...
import repcom.tela.quadro.base.MesAnoQuadroListaBase;

public abstract class MesAnoListUsoAdapterBase extends BaseAdapter {
	
	private DigicomContexto context;
	private List<MesAno> lista;
	...
	public MesAnoListUsoAdapterBase(List<MesAno> lista, DigicomContexto context ) {
		this.context = context;
		this.lista = lista;
		setEdicao(); // Forcando para fazer edicao - alterar se for necessario depois.
	}
	
	public void setListaNova(List<MesAno> listaNova) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		MesAno item = lista.get(position);
		...
		if (!(saida instanceof ItemLista))
			throw new RuntimeException("Alterar layout raiz de item_mesano.xml para br.com.digicom.layout.ItemLista");
	...
	protected void montaItemLista(int position, MesAno item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_mesano");
	}
	protected ResourceObj getLayoutItem() {
		return new ResourceObj(R.layout.item_mesano,"R.layout.item_mesano");
	}
	/*
	@Override
	public boolean onTouch(View view, MotionEvent avent) {
	 	ItemLista objeto = (ItemLista) view;
		MesAno item = (MesAno) objeto.getItem();
