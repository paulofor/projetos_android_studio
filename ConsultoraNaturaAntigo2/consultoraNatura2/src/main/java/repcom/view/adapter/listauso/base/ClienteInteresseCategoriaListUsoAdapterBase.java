...
import repcom.view.ClienteInteresseCategoriaView;
...
public abstract class ClienteInteresseCategoriaListUsoAdapterBase extends BaseAdapter {
	
	private DigicomContexto context;
	private List<ClienteInteresseCategoria> lista;
	...
	public ClienteInteresseCategoriaListUsoAdapterBase(List<ClienteInteresseCategoria> lista, DigicomContexto context ) {
		this.context = context;
		this.lista = lista;
		setEdicao(); // Forcando para fazer edicao - alterar se for necessario depois.
	}
	
	public void setListaNova(List<ClienteInteresseCategoria> listaNova) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ClienteInteresseCategoria item = lista.get(position);
		...
		if (!(saida instanceof ItemLista))
			throw new RuntimeException("Alterar layout raiz de item_cliente_interesse_categoria.xml para br.com.digicom.layout.ItemLista");
	...
	protected void montaItemLista(int position, ClienteInteresseCategoria item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_cliente_interesse_categoria");
	}
	protected ResourceObj getLayoutItem() {
		return new ResourceObj(R.layout.item_cliente_interesse_categoria,"R.layout.item_cliente_interesse_categoria");
	}
	/*
	@Override
	public boolean onTouch(View view, MotionEvent avent) {
	 	ItemLista objeto = (ItemLista) view;
		ClienteInteresseCategoria item = (ClienteInteresseCategoria) objeto.getItem();
