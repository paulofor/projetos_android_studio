...
import repcom.view.ContatoClienteView;
...
public abstract class ContatoClienteListUsoAdapterBase extends BaseAdapter {
	
	private DigicomContexto context;
	private List<ContatoCliente> lista;
	...
	public ContatoClienteListUsoAdapterBase(List<ContatoCliente> lista, DigicomContexto context ) {
		this.context = context;
		this.lista = lista;
		setEdicao(); // Forcando para fazer edicao - alterar se for necessario depois.
	}
	
	public void setListaNova(List<ContatoCliente> listaNova) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ContatoCliente item = lista.get(position);
		...
		if (!(saida instanceof ItemLista))
			throw new RuntimeException("Alterar layout raiz de item_contato_cliente.xml para br.com.digicom.layout.ItemLista");
	...
	protected void montaItemLista(int position, ContatoCliente item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_contato_cliente");
	}
	protected ResourceObj getLayoutItem() {
		return new ResourceObj(R.layout.item_contato_cliente,"R.layout.item_contato_cliente");
	}
	/*
	@Override
	public boolean onTouch(View view, MotionEvent avent) {
	 	ItemLista objeto = (ItemLista) view;
		ContatoCliente item = (ContatoCliente) objeto.getItem();
