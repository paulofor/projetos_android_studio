...
public class EstoqueMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public EstoqueMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public EstoqueMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		Estoque item = null;
		item = (Estoque) entrada;item.setIdEstoque(getInt(c,pos++));
		if (sinc) {
