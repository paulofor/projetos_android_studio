...
public class CargaPlanejadaSincronismo {
    ...
    public void sincroniza(Context contexto, boolean forcaAtualizacao, boolean delete, boolean atualizaLocal) {
    	String codigoAplicacao = AplicacaoContract.getCodigoAplicacaoSinc();
 	 	CargaPlanejadaRemote servicoRemoto = new  CargaPlanejadaRemote(); // fazer estatico ?
        try {
             ContentResolver resolver = contexto.getContentResolver();
             Cursor cursor = resolver.query(CargaPlanejadaContract.buildAllSinc(), CargaPlanejadaContract.COLS_SINC, null, null, null);
             int tam = cursor.getCount();
             if (cursor.getCount() > 0 ) {
                  servicoRemoto.listaSincronizadaAlteracao(cursor, contexto);
                  DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "CargaPlanejada: " + tam +  " >> ");
                  resolver.delete(CargaPlanejadaContract.buildDeleteAllSinc(),null,null);
             }
