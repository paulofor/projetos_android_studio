...
public abstract class DiaTreinoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 748 * 10;
    private long idDiaTreino = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idDiaTreino = extra.getInt(Constantes.DIA_TREINO_ID);
        } catch (Exception e) {
            idDiaTreino = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idDiaTreino==0) {
            throw new RuntimeException("Nao recebeu o IdDiaTreino");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idDiaTreino);
        edt.commit();
        ...
        complementaOnCreate(idDiaTreino);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = DiaTreinoContract.buildDiaTreinoUri(idDiaTreino);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(DiaTreinoContract.COM_COMPLEMENTO).build();
        uri = DiaTreinoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return DiaTreinoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = DiaTreinoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        DiaTreino item = (DiaTreino) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(DiaTreino item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idDiaTreino);
