...
import br.com.lojadigicom.treinoacademia.modelo.ExecucaoItemSerieVo;
...
public class ExecucaoItemSerieMontador implements MontadorDaoI {

	private ExecucaoItemSerie principal = new ExecucaoItemSerieVo();
	
	private boolean sinc = false;
	public ExecucaoItemSerieMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public ExecucaoItemSerieMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c) {
		...
		//objeto = FabricaVo.criaExecucaoItemSerie();
		objeto = new ExecucaoItemSerieVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (ExecucaoItemSerie) entrada;
		item.setIdExecucaoItemSerie(getInt(c,pos++));
		item.setDataHoraInicio(getTimestampDataHora(c,pos++));
		item.setCargaUtilizada(getFloat(c,pos++));
		item.setSucessoRepeticoes(getLogic(c,pos++));
		item.setNumeroSerie(getInt(c,pos++));
		item.setDataHoraFinalizacao(getTimestampDataHora(c,pos++));
		item.setQuantidadeRepeticao(getInt(c,pos++));
		item.setIdItemSerieRa(getInt(c,pos++));
		item.setIdDiaTreinoE(getInt(c,pos++));
		item.setIdExercicioRa(getInt(c,pos++));
