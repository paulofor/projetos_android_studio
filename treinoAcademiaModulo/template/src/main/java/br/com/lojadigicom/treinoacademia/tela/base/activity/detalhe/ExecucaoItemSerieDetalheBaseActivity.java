...
public abstract class ExecucaoItemSerieDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 747 * 10;
    private long idExecucaoItemSerie = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idExecucaoItemSerie = extra.getInt(Constantes.EXECUCAO_ITEM_SERIE_ID);
        } catch (Exception e) {
            idExecucaoItemSerie = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idExecucaoItemSerie==0) {
            throw new RuntimeException("Nao recebeu o IdExecucaoItemSerie");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idExecucaoItemSerie);
        edt.commit();
        ...
        complementaOnCreate(idExecucaoItemSerie);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ExecucaoItemSerieContract.buildExecucaoItemSerieUri(idExecucaoItemSerie);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ExecucaoItemSerieContract.COM_COMPLEMENTO).build();
        uri = ExecucaoItemSerieContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ExecucaoItemSerieContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ExecucaoItemSerieContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        ExecucaoItemSerie item = (ExecucaoItemSerie) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(ExecucaoItemSerie item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idExecucaoItemSerie);
