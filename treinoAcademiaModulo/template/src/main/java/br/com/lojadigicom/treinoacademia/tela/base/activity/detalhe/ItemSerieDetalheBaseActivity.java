...
public abstract class ItemSerieDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 745 * 10;
    private long idItemSerie = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idItemSerie = extra.getInt(Constantes.ITEM_SERIE_ID);
        } catch (Exception e) {
            idItemSerie = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idItemSerie==0) {
            throw new RuntimeException("Nao recebeu o IdItemSerie");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idItemSerie);
        edt.commit();
        ...
        complementaOnCreate(idItemSerie);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ItemSerieContract.buildItemSerieUri(idItemSerie);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ItemSerieContract.COM_COMPLEMENTO).build();
        uri = ItemSerieContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ItemSerieContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ItemSerieContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        ItemSerie item = (ItemSerie) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(ItemSerie item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idItemSerie);
