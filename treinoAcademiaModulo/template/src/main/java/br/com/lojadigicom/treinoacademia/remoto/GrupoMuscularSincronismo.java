...
public class GrupoMuscularSincronismo {
    ...
    public void sincroniza(Context contexto, boolean forcaAtualizacao, boolean delete, boolean atualizaLocal) {
    	String codigoAplicacao = AplicacaoContract.getCodigoAplicacaoSinc();
 	 	GrupoMuscularRemote servicoRemoto = new  GrupoMuscularRemote(); // fazer estatico ?
        try {
             ContentResolver resolver = contexto.getContentResolver();
             Cursor cursor = resolver.query(GrupoMuscularContract.buildAllSinc(), GrupoMuscularContract.COLS_SINC, null, null, null);
             int tam = cursor.getCount();
             if (cursor.getCount() > 0 ) {
                  servicoRemoto.listaSincronizadaAlteracao(cursor, contexto);
                  DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "GrupoMuscular: " + tam +  " >> ");
                  resolver.delete(GrupoMuscularContract.buildDeleteAllSinc(),null,null);
             }
