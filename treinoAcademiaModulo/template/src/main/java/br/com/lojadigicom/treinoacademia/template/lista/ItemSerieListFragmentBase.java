...
public abstract class ItemSerieListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ItemSerieListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 745;
	...
	public ItemSerieListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ItemSerieListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.ITEM_SERIE_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ItemSerieListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ItemSerieListAdapterBase.ItemSerieListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ItemSerieListViewHolderBase vh) {
    ...
    protected abstract ItemSerieListAdapterBase getListAdapter(FragmentActivity activity, ItemSerieListAdapterBase.ItemSerieListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ItemSerieListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.ITEM_SERIE_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.ITEM_SERIE_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ItemSerie> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ItemSerie " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ItemSerie> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ItemSerieContract.getMontador();
    }

    protected final List<ItemSerie> converteLista(Cursor data) {
	...
	protected long mIdSerieTreino;
	public final void setIdSerieTreino(long id) {
		mIdSerieTreino = id;
	}
