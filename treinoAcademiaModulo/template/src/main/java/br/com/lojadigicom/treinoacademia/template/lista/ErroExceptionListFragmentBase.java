...
public abstract class ErroExceptionListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ErroExceptionListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 853;
	...
	public ErroExceptionListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ErroExceptionListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.ERRO_EXCEPTION_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ErroExceptionListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ErroExceptionListAdapterBase.ErroExceptionListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ErroExceptionListViewHolderBase vh) {
    ...
    protected abstract ErroExceptionListAdapterBase getListAdapter(FragmentActivity activity, ErroExceptionListAdapterBase.ErroExceptionListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ErroExceptionListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.ERRO_EXCEPTION_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.ERRO_EXCEPTION_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ErroException> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ErroException " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ErroException> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ErroExceptionContract.getMontador();
    }

    protected final List<ErroException> converteLista(Cursor data) {
