...
public abstract class GrupoMuscularDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 848 * 10;
    private long idGrupoMuscular = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idGrupoMuscular = extra.getInt(Constantes.GRUPO_MUSCULAR_ID);
        } catch (Exception e) {
            idGrupoMuscular = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idGrupoMuscular==0) {
            throw new RuntimeException("Nao recebeu o IdGrupoMuscular");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idGrupoMuscular);
        edt.commit();
        ...
        complementaOnCreate(idGrupoMuscular);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = GrupoMuscularContract.buildGrupoMuscularUri(idGrupoMuscular);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(GrupoMuscularContract.COM_COMPLEMENTO).build();
        uri = GrupoMuscularContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return GrupoMuscularContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = GrupoMuscularContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        GrupoMuscular item = (GrupoMuscular) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(GrupoMuscular item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idGrupoMuscular);
