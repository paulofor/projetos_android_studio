 ...
 */
public abstract class ItemSerieListAdapterBase extends RecyclerView.Adapter<ItemSerieListViewHolderBase>{

    //protected Cursor mCursor;
    protected List<ItemSerie> mLista;
    private Context mContext;
    private ItemSerieListAdapterOnClickHandler mClickHandler;
    private View mEmptyView;
    private ItemChoiceManager mICM = null;
    protected ResourceObj resourceObj = getLayoutItemResource();

    public ItemSerieListAdapterBase(Context context, ItemSerieListAdapterOnClickHandler dh, View emptyView, int choiceMode) {
        mContext = context;
        mClickHandler = dh;
    ...
    @Override
    public final ItemSerieListViewHolderBase onCreateViewHolder(ViewGroup parent, int viewType) {
        if ( parent instanceof RecyclerView ) {
            ...
            ItemSerieListViewHolderBase saida = getViewHolder(view);
    ...
    protected abstract ItemSerieListViewHolderBase getViewHolder(View v);
    ...
    public final void swapCursor(List<ItemSerie> novaLista) {
        //mCursor = newCursor;
        mLista = novaLista;
    ...
    public static interface ItemSerieListAdapterOnClickHandler {
        void onClick(int id, ItemSerieListViewHolderBase vh);
    }
    
    
    @Override
    public void onBindViewHolder(ItemSerieListViewHolderBase holder, int position) {
        ItemSerie obj = mLista.get(position);
        holder.setItem(obj);
        onBindViewHolderDC(obj,holder,position);
    }

    protected abstract void onBindViewHolderDC(ItemSerie item,ItemSerieListViewHolderBase holder, int position);
