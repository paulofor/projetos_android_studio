...
public abstract class SerieTreinoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private SerieTreinoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 744;
	...
	public SerieTreinoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, SerieTreinoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.SERIE_TREINO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(SerieTreinoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new SerieTreinoListAdapterBase.SerieTreinoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, SerieTreinoListViewHolderBase vh) {
    ...
    protected abstract SerieTreinoListAdapterBase getListAdapter(FragmentActivity activity, SerieTreinoListAdapterBase.SerieTreinoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(SerieTreinoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.SERIE_TREINO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.SERIE_TREINO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<SerieTreino> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista SerieTreino " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<SerieTreino> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return SerieTreinoContract.getMontador();
    }

    protected final List<SerieTreino> converteLista(Cursor data) {
