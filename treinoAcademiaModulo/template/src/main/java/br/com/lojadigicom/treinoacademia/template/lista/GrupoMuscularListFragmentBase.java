...
public abstract class GrupoMuscularListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private GrupoMuscularListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 848;
	...
	public GrupoMuscularListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, GrupoMuscularListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.GRUPO_MUSCULAR_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(GrupoMuscularListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new GrupoMuscularListAdapterBase.GrupoMuscularListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, GrupoMuscularListViewHolderBase vh) {
    ...
    protected abstract GrupoMuscularListAdapterBase getListAdapter(FragmentActivity activity, GrupoMuscularListAdapterBase.GrupoMuscularListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(GrupoMuscularListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.GRUPO_MUSCULAR_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.GRUPO_MUSCULAR_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<GrupoMuscular> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista GrupoMuscular " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<GrupoMuscular> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return GrupoMuscularContract.getMontador();
    }

    protected final List<GrupoMuscular> converteLista(Cursor data) {
