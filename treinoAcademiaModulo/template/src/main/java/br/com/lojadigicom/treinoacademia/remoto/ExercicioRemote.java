...
public class ExercicioRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ExercicioContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ExercicioContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_exercicio=" + valor.getAsString("id_exercicio");
                contexto.getContentResolver().update(ExercicioContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdExercicio",c.getString(ExercicioContract.COL_ID_EXERCICIO));
		
			
		
		json.put("DescricaoExercicio",c.getString(ExercicioContract.COL_DESCRICAO_EXERCICIO));
		
			
		
		json.put("Imagem",c.getString(ExercicioContract.COL_IMAGEM));
		
			
		
		json.put("Titulo",c.getString(ExercicioContract.COL_TITULO));
		
			
		
		json.put("Subtitulo",c.getString(ExercicioContract.COL_SUBTITULO));
		
			
		
		json.put("IdGrupoMuscularP",c.getString(ExercicioContract.COL_ID_GRUPO_MUSCULAR_P));
		
			
		
		json.put("IdUsuarioS",c.getString(ExercicioContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ExercicioContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ExercicioContract.COLUNA_ID_EXERCICIO, obj.getString("IdExercicio"));
		
		valores.put(ExercicioContract.COLUNA_DESCRICAO_EXERCICIO, obj.getString("DescricaoExercicio"));
		
		valores.put(ExercicioContract.COLUNA_IMAGEM, obj.getString("Imagem"));
		
		valores.put(ExercicioContract.COLUNA_TITULO, obj.getString("Titulo"));
		
		valores.put(ExercicioContract.COLUNA_SUBTITULO, obj.getString("Subtitulo"));
		
		valores.put(ExercicioContract.COLUNA_ID_GRUPO_MUSCULAR_P, obj.getString("IdGrupoMuscularP"));
		
		valores.put(ExercicioContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
