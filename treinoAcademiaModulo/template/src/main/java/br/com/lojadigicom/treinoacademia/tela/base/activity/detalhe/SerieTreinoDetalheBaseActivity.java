...
public abstract class SerieTreinoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 744 * 10;
    private long idSerieTreino = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idSerieTreino = extra.getInt(Constantes.SERIE_TREINO_ID);
        } catch (Exception e) {
            idSerieTreino = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idSerieTreino==0) {
            throw new RuntimeException("Nao recebeu o IdSerieTreino");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idSerieTreino);
        edt.commit();
        ...
        complementaOnCreate(idSerieTreino);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = SerieTreinoContract.buildSerieTreinoUri(idSerieTreino);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(SerieTreinoContract.COM_COMPLEMENTO).build();
        uri = SerieTreinoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return SerieTreinoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = SerieTreinoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        SerieTreino item = (SerieTreino) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(SerieTreino item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idSerieTreino);
