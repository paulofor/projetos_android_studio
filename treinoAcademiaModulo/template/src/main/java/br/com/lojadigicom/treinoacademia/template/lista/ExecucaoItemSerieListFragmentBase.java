...
import br.com.lojadigicom.treinoacademia.framework.tela.DCAplicacao;
...
import br.com.lojadigicom.treinoacademia.framework.tela.SimpleDividerItemDecoration;

public abstract class ExecucaoItemSerieListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ExecucaoItemSerieListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 747;
	...
	public ExecucaoItemSerieListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ExecucaoItemSerieListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.EXECUCAO_ITEM_SERIE_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ExecucaoItemSerieListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ExecucaoItemSerieListAdapterBase.ExecucaoItemSerieListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ExecucaoItemSerieListViewHolderBase vh) {
		...
		SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
    ...
    protected abstract ExecucaoItemSerieListAdapterBase getListAdapter(FragmentActivity activity, ExecucaoItemSerieListAdapterBase.ExecucaoItemSerieListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ExecucaoItemSerieListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.EXECUCAO_ITEM_SERIE_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.EXECUCAO_ITEM_SERIE_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ExecucaoItemSerie> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ExecucaoItemSerie " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ExecucaoItemSerie> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ExecucaoItemSerieContract.getMontador();
    }

    protected final List<ExecucaoItemSerie> converteLista(Cursor data) {
	...
	protected long mIdItemSerie;
	public final void setIdItemSerie(long id) {
		mIdItemSerie = id;
	}
	
	protected long mIdDiaTreino;
	public final void setIdDiaTreino(long id) {
		mIdDiaTreino = id;
	}
	
	protected long mIdExercicio;
	public final void setIdExercicio(long id) {
		mIdExercicio = id;
	}
