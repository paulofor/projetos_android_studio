...
public class CargaPlanejadaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CargaPlanejadaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CargaPlanejadaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_carga_planejada=" + valor.getAsString("id_carga_planejada");
                contexto.getContentResolver().update(CargaPlanejadaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCargaPlanejada",c.getString(CargaPlanejadaContract.COL_ID_CARGA_PLANEJADA));
		
			
		
		json.put("ValorCarga",c.getString(CargaPlanejadaContract.COL_VALOR_CARGA));
		
			
		
		json.put("DataInicio",c.getString(CargaPlanejadaContract.COL_DATA_INICIO));
		
			
		
		json.put("DataFinal",c.getString(CargaPlanejadaContract.COL_DATA_FINAL));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Ativa",c.getInt(CargaPlanejadaContract.COL_ATIVA)==1);
		
			
		
		json.put("QuantidadeRepeticao",c.getString(CargaPlanejadaContract.COL_QUANTIDADE_REPETICAO));
		
			
		
		json.put("OrdemRepeticao",c.getString(CargaPlanejadaContract.COL_ORDEM_REPETICAO));
		
			
		
		json.put("IdItemSerieRa",c.getString(CargaPlanejadaContract.COL_ID_ITEM_SERIE_RA));
		
			
		
		json.put("IdUsuarioS",c.getString(CargaPlanejadaContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CargaPlanejadaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CargaPlanejadaContract.COLUNA_ID_CARGA_PLANEJADA, obj.getString("IdCargaPlanejada"));
		
		valores.put(CargaPlanejadaContract.COLUNA_VALOR_CARGA, obj.getString("ValorCarga"));
		
		valores.put(CargaPlanejadaContract.COLUNA_DATA_INICIO, obj.getLong("DataInicio"));
		
		valores.put(CargaPlanejadaContract.COLUNA_DATA_FINAL, obj.getLong("DataFinal"));
			
		int valAtiva = 0;
		if ("True".equals(obj.getString("Ativa"))) {
			valAtiva = 1;
		}
		valores.put(CargaPlanejadaContract.COLUNA_ATIVA, valAtiva);
		
		valores.put(CargaPlanejadaContract.COLUNA_QUANTIDADE_REPETICAO, obj.getString("QuantidadeRepeticao"));
		
		valores.put(CargaPlanejadaContract.COLUNA_ORDEM_REPETICAO, obj.getString("OrdemRepeticao"));
		
		valores.put(CargaPlanejadaContract.COLUNA_ID_ITEM_SERIE_RA, obj.getString("IdItemSerieRa"));
		
		valores.put(CargaPlanejadaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
