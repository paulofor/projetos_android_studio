...
public class CargaPlanejadaMontador implements MontadorDaoI {

	private CargaPlanejada principal = new CargaPlanejadaVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (CargaPlanejada) entrada;
		item.setIdCargaPlanejada(getInt(c,pos++));
		item.setValorCarga(getFloat(c,pos++));
		item.setDataInicio(this.getTimestampData(c,pos++));
		item.setDataFinal(this.getTimestampData(c,pos++));
