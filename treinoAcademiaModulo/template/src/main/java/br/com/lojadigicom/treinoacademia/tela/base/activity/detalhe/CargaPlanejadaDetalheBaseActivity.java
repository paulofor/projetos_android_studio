...
public abstract class CargaPlanejadaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 746 * 10;
    private long idCargaPlanejada = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idCargaPlanejada = extra.getInt(Constantes.CARGA_PLANEJADA_ID);
        } catch (Exception e) {
            idCargaPlanejada = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idCargaPlanejada==0) {
            throw new RuntimeException("Nao recebeu o IdCargaPlanejada");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idCargaPlanejada);
        edt.commit();
        ...
        complementaOnCreate(idCargaPlanejada);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = CargaPlanejadaContract.buildCargaPlanejadaUri(idCargaPlanejada);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(CargaPlanejadaContract.COM_COMPLEMENTO).build();
        uri = CargaPlanejadaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return CargaPlanejadaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = CargaPlanejadaContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        CargaPlanejada item = (CargaPlanejada) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(CargaPlanejada item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idCargaPlanejada);
