...
public abstract class ExercicioDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 743 * 10;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idExercicio = extra.getInt(Constantes.EXERCICIO_ID);
        } catch (Exception e) {
            idExercicio = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idExercicio==0) {
            throw new RuntimeException("Nao recebeu o IdExercicio");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idExercicio);
        edt.commit();
        ...
        complementaOnCreate(idExercicio);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ExercicioContract.buildExercicioUri(idExercicio);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ExercicioContract.COM_COMPLEMENTO).build();
        uri = ExercicioContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ExercicioContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ExercicioContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        Exercicio item = (Exercicio) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(Exercicio item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idExercicio);
