...
public class ExecucaoItemSerieRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ExecucaoItemSerieContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ExecucaoItemSerieContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_execucao_item_serie=" + valor.getAsString("id_execucao_item_serie");
                contexto.getContentResolver().update(ExecucaoItemSerieContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdExecucaoItemSerie",c.getString(ExecucaoItemSerieContract.COL_ID_EXECUCAO_ITEM_SERIE));
		
			
		
		json.put("DataHoraInicio",c.getString(ExecucaoItemSerieContract.COL_DATA_HORA_INICIO));
		
			
		
		json.put("CargaUtilizada",c.getString(ExecucaoItemSerieContract.COL_CARGA_UTILIZADA));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("SucessoRepeticoes",c.getInt(ExecucaoItemSerieContract.COL_SUCESSO_REPETICOES)==1);
		
			
		
		json.put("NumeroSerie",c.getString(ExecucaoItemSerieContract.COL_NUMERO_SERIE));
		
			
		
		json.put("DataHoraFinalizacao",c.getString(ExecucaoItemSerieContract.COL_DATA_HORA_FINALIZACAO));
		
			
		
		json.put("QuantidadeRepeticao",c.getString(ExecucaoItemSerieContract.COL_QUANTIDADE_REPETICAO));
		
			
		
		json.put("IdItemSerieRa",c.getString(ExecucaoItemSerieContract.COL_ID_ITEM_SERIE_RA));
		
			
		
		json.put("IdDiaTreinoE",c.getString(ExecucaoItemSerieContract.COL_ID_DIA_TREINO_E));
		
			
		
		json.put("IdExercicioRa",c.getString(ExecucaoItemSerieContract.COL_ID_EXERCICIO_RA));
		
			
		
		json.put("IdUsuarioS",c.getString(ExecucaoItemSerieContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ExecucaoItemSerieContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ExecucaoItemSerieContract.COLUNA_ID_EXECUCAO_ITEM_SERIE, obj.getString("IdExecucaoItemSerie"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_DATA_HORA_INICIO, obj.getLong("DataHoraInicio"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_CARGA_UTILIZADA, obj.getString("CargaUtilizada"));
			
		int valSucessoRepeticoes = 0;
		if ("True".equals(obj.getString("SucessoRepeticoes"))) {
			valSucessoRepeticoes = 1;
		}
		valores.put(ExecucaoItemSerieContract.COLUNA_SUCESSO_REPETICOES, valSucessoRepeticoes);
		
		valores.put(ExecucaoItemSerieContract.COLUNA_NUMERO_SERIE, obj.getString("NumeroSerie"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_DATA_HORA_FINALIZACAO, obj.getLong("DataHoraFinalizacao"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_QUANTIDADE_REPETICAO, obj.getString("QuantidadeRepeticao"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_ID_ITEM_SERIE_RA, obj.getString("IdItemSerieRa"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_ID_DIA_TREINO_E, obj.getString("IdDiaTreinoE"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_ID_EXERCICIO_RA, obj.getString("IdExercicioRa"));
		
		valores.put(ExecucaoItemSerieContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
