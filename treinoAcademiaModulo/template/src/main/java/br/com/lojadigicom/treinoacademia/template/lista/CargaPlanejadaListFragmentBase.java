...
public abstract class CargaPlanejadaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private CargaPlanejadaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 746;
	...
	public CargaPlanejadaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, CargaPlanejadaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CARGA_PLANEJADA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(CargaPlanejadaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new CargaPlanejadaListAdapterBase.CargaPlanejadaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, CargaPlanejadaListViewHolderBase vh) {
    ...
    protected abstract CargaPlanejadaListAdapterBase getListAdapter(FragmentActivity activity, CargaPlanejadaListAdapterBase.CargaPlanejadaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(CargaPlanejadaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CARGA_PLANEJADA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CARGA_PLANEJADA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<CargaPlanejada> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista CargaPlanejada " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<CargaPlanejada> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return CargaPlanejadaContract.getMontador();
    }

    protected final List<CargaPlanejada> converteLista(Cursor data) {
