...
public class ItemSerieRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ItemSerieContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ItemSerieContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_item_serie=" + valor.getAsString("id_item_serie");
                contexto.getContentResolver().update(ItemSerieContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdItemSerie",c.getString(ItemSerieContract.COL_ID_ITEM_SERIE));
		
			
		
		json.put("Repeticoes",c.getString(ItemSerieContract.COL_REPETICOES));
		
			
		
		json.put("Serie",c.getString(ItemSerieContract.COL_SERIE));
		
			
		
		json.put("Parametros",c.getString(ItemSerieContract.COL_PARAMETROS));
		
			
		
		json.put("OrdemExecucao",c.getString(ItemSerieContract.COL_ORDEM_EXECUCAO));
		
			
		
		json.put("IdExercicioEd",c.getString(ItemSerieContract.COL_ID_EXERCICIO_ED));
		
			
		
		json.put("IdSerieTreinoPa",c.getString(ItemSerieContract.COL_ID_SERIE_TREINO_PA));
		
			
		
		json.put("IdUsuarioS",c.getString(ItemSerieContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ItemSerieContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ItemSerieContract.COLUNA_ID_ITEM_SERIE, obj.getString("IdItemSerie"));
		
		valores.put(ItemSerieContract.COLUNA_REPETICOES, obj.getString("Repeticoes"));
		
		valores.put(ItemSerieContract.COLUNA_SERIE, obj.getString("Serie"));
		
		valores.put(ItemSerieContract.COLUNA_PARAMETROS, obj.getString("Parametros"));
		
		valores.put(ItemSerieContract.COLUNA_ORDEM_EXECUCAO, obj.getString("OrdemExecucao"));
		
		valores.put(ItemSerieContract.COLUNA_ID_EXERCICIO_ED, obj.getString("IdExercicioEd"));
		
		valores.put(ItemSerieContract.COLUNA_ID_SERIE_TREINO_PA, obj.getString("IdSerieTreinoPa"));
		
		valores.put(ItemSerieContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
