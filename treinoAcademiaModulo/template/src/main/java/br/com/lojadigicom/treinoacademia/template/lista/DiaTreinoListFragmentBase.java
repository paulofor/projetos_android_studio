...
public abstract class DiaTreinoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private DiaTreinoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 748;
	...
	public DiaTreinoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, DiaTreinoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.DIA_TREINO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(DiaTreinoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new DiaTreinoListAdapterBase.DiaTreinoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, DiaTreinoListViewHolderBase vh) {
    ...
    protected abstract DiaTreinoListAdapterBase getListAdapter(FragmentActivity activity, DiaTreinoListAdapterBase.DiaTreinoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(DiaTreinoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.DIA_TREINO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.treinoacademia", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.DIA_TREINO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<DiaTreino> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista DiaTreino " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<DiaTreino> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return DiaTreinoContract.getMontador();
    }

    protected final List<DiaTreino> converteLista(Cursor data) {
