...
public class SerieTreinoRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(SerieTreinoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(SerieTreinoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_serie_treino=" + valor.getAsString("id_serie_treino");
                contexto.getContentResolver().update(SerieTreinoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdSerieTreino",c.getString(SerieTreinoContract.COL_ID_SERIE_TREINO));
		
			
		
		json.put("QtdeExecucao",c.getString(SerieTreinoContract.COL_QTDE_EXECUCAO));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Ativa",c.getInt(SerieTreinoContract.COL_ATIVA)==1);
		
			
		
		json.put("DataCriacao",c.getString(SerieTreinoContract.COL_DATA_CRIACAO));
		
			
		
		json.put("DataUltimaExecucao",c.getString(SerieTreinoContract.COL_DATA_ULTIMA_EXECUCAO));
		
			
		
		json.put("IdUsuarioS",c.getString(SerieTreinoContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(SerieTreinoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(SerieTreinoContract.COLUNA_ID_SERIE_TREINO, obj.getString("IdSerieTreino"));
		
		valores.put(SerieTreinoContract.COLUNA_QTDE_EXECUCAO, obj.getString("QtdeExecucao"));
			
		int valAtiva = 0;
		if ("True".equals(obj.getString("Ativa"))) {
			valAtiva = 1;
		}
		valores.put(SerieTreinoContract.COLUNA_ATIVA, valAtiva);
		
		valores.put(SerieTreinoContract.COLUNA_DATA_CRIACAO, obj.getLong("DataCriacao"));
		
		valores.put(SerieTreinoContract.COLUNA_DATA_ULTIMA_EXECUCAO, obj.getLong("DataUltimaExecucao"));
		
		valores.put(SerieTreinoContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
