...
public abstract class PrecoProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PrecoProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 677;
	...
	public PrecoProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, PrecoProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PRECO_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(PrecoProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new PrecoProdutoListAdapterBase.PrecoProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PrecoProdutoListViewHolderBase vh) {
    ...
    protected abstract PrecoProdutoListAdapterBase getListAdapter(FragmentActivity activity, PrecoProdutoListAdapterBase.PrecoProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PrecoProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRECO_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRECO_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PrecoProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PrecoProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<PrecoProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PrecoProdutoContract.getMontador();
    }

    protected final List<PrecoProduto> converteLista(Cursor data) {
