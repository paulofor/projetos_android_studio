...
public class ParcelaVendaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ParcelaVendaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ParcelaVendaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_parcela_venda=" + valor.getAsString("id_parcela_venda");
                contexto.getContentResolver().update(ParcelaVendaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdParcelaVenda",c.getString(ParcelaVendaContract.COL_ID_PARCELA_VENDA));
		
			
		
		json.put("ValorParcela",c.getString(ParcelaVendaContract.COL_VALOR_PARCELA));
		
			
		
		json.put("DataVencimento",c.getString(ParcelaVendaContract.COL_DATA_VENCIMENTO));
		
			
		
		json.put("NumeroParcela",c.getString(ParcelaVendaContract.COL_NUMERO_PARCELA));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Paga",c.getInt(ParcelaVendaContract.COL_PAGA)==1);
		
			
		
		json.put("IdVendaPa",c.getString(ParcelaVendaContract.COL_ID_VENDA_PA));
		
			
		
		json.put("IdUsuarioS",c.getString(ParcelaVendaContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ParcelaVendaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ParcelaVendaContract.COLUNA_ID_PARCELA_VENDA, obj.getString("IdParcelaVenda"));
		
		valores.put(ParcelaVendaContract.COLUNA_VALOR_PARCELA, obj.getString("ValorParcela"));
		
		valores.put(ParcelaVendaContract.COLUNA_DATA_VENCIMENTO, obj.getLong("DataVencimento"));
		
		valores.put(ParcelaVendaContract.COLUNA_NUMERO_PARCELA, obj.getString("NumeroParcela"));
			
		int valPaga = 0;
		if ("True".equals(obj.getString("Paga"))) {
			valPaga = 1;
		}
		valores.put(ParcelaVendaContract.COLUNA_PAGA, valPaga);
		
		valores.put(ParcelaVendaContract.COLUNA_ID_VENDA_PA, obj.getString("IdVendaPa"));
		
		valores.put(ParcelaVendaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
