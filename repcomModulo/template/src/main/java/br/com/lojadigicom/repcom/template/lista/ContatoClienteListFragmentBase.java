...
public abstract class ContatoClienteListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ContatoClienteListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 401;
	...
	public ContatoClienteListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ContatoClienteListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CONTATO_CLIENTE_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ContatoClienteListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ContatoClienteListAdapterBase.ContatoClienteListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ContatoClienteListViewHolderBase vh) {
    ...
    protected abstract ContatoClienteListAdapterBase getListAdapter(FragmentActivity activity, ContatoClienteListAdapterBase.ContatoClienteListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ContatoClienteListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CONTATO_CLIENTE_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CONTATO_CLIENTE_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ContatoCliente> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ContatoCliente " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ContatoCliente> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ContatoClienteContract.getMontador();
    }

    protected final List<ContatoCliente> converteLista(Cursor data) {
