...
public abstract class ParcelaVendaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ParcelaVendaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 406;
	...
	public ParcelaVendaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ParcelaVendaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PARCELA_VENDA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ParcelaVendaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ParcelaVendaListAdapterBase.ParcelaVendaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ParcelaVendaListViewHolderBase vh) {
    ...
    protected abstract ParcelaVendaListAdapterBase getListAdapter(FragmentActivity activity, ParcelaVendaListAdapterBase.ParcelaVendaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ParcelaVendaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PARCELA_VENDA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PARCELA_VENDA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ParcelaVenda> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ParcelaVenda " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ParcelaVenda> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ParcelaVendaContract.getMontador();
    }

    protected final List<ParcelaVenda> converteLista(Cursor data) {
	...
	protected long mIdVenda;
	public final void setIdVenda(long id) {
		mIdVenda = id;
	}
