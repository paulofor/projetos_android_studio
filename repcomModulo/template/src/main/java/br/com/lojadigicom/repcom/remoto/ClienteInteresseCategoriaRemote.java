...
public class ClienteInteresseCategoriaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ClienteInteresseCategoriaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ClienteInteresseCategoriaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_cliente_interesse_categoria=" + valor.getAsString("id_cliente_interesse_categoria");
                contexto.getContentResolver().update(ClienteInteresseCategoriaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdClienteInteresseCategoria",c.getString(ClienteInteresseCategoriaContract.COL_ID_CLIENTE_INTERESSE_CATEGORIA));
		
			
		
		json.put("IdClienteA",c.getString(ClienteInteresseCategoriaContract.COL_ID_CLIENTE_A));
		
			
		
		json.put("IdCategoriaProdutoA",c.getString(ClienteInteresseCategoriaContract.COL_ID_CATEGORIA_PRODUTO_A));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ClienteInteresseCategoriaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ClienteInteresseCategoriaContract.COLUNA_ID_CLIENTE_INTERESSE_CATEGORIA, obj.getString("IdClienteInteresseCategoria"));
		
		valores.put(ClienteInteresseCategoriaContract.COLUNA_ID_CLIENTE_A, obj.getString("IdClienteA"));
		
		valores.put(ClienteInteresseCategoriaContract.COLUNA_ID_CATEGORIA_PRODUTO_A, obj.getString("IdCategoriaProdutoA"));
