...
public abstract class ProdutoVendaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 404 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ProdutoVendaContract.COM_COMPLEMENTO).build();
        uri = ProdutoVendaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ProdutoVendaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
