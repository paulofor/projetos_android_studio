...
public class ProdutoVendaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ProdutoVendaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ProdutoVendaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_produto_venda=" + valor.getAsString("id_produto_venda");
                contexto.getContentResolver().update(ProdutoVendaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdProdutoVenda",c.getString(ProdutoVendaContract.COL_ID_PRODUTO_VENDA));
		
			
		
		json.put("Quantidade",c.getString(ProdutoVendaContract.COL_QUANTIDADE));
		
			
		
		json.put("ValorTotal",c.getString(ProdutoVendaContract.COL_VALOR_TOTAL));
		
			
		
		json.put("ValorItem",c.getString(ProdutoVendaContract.COL_VALOR_ITEM));
		
			
		
		json.put("IdProdutoA",c.getString(ProdutoVendaContract.COL_ID_PRODUTO_A));
		
			
		
		json.put("IdVendaA",c.getString(ProdutoVendaContract.COL_ID_VENDA_A));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ProdutoVendaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ProdutoVendaContract.COLUNA_ID_PRODUTO_VENDA, obj.getString("IdProdutoVenda"));
		
		valores.put(ProdutoVendaContract.COLUNA_QUANTIDADE, obj.getString("Quantidade"));
		
		valores.put(ProdutoVendaContract.COLUNA_VALOR_TOTAL, obj.getString("ValorTotal"));
		
		valores.put(ProdutoVendaContract.COLUNA_VALOR_ITEM, obj.getString("ValorItem"));
		
		valores.put(ProdutoVendaContract.COLUNA_ID_PRODUTO_A, obj.getString("IdProdutoA"));
		
		valores.put(ProdutoVendaContract.COLUNA_ID_VENDA_A, obj.getString("IdVendaA"));
