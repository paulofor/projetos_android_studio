...
public class CategoriaProdutoProdutoRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CategoriaProdutoProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CategoriaProdutoProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_categoria_produto_produto=" + valor.getAsString("id_categoria_produto_produto");
                contexto.getContentResolver().update(CategoriaProdutoProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCategoriaProdutoProduto",c.getString(CategoriaProdutoProdutoContract.COL_ID_CATEGORIA_PRODUTO_PRODUTO));
		
			
		
		json.put("DataInclusao",c.getString(CategoriaProdutoProdutoContract.COL_DATA_INCLUSAO));
		
			
		
		json.put("IdCategoriaProdutoRa",c.getString(CategoriaProdutoProdutoContract.COL_ID_CATEGORIA_PRODUTO_RA));
		
			
		
		json.put("IdProdutoRa",c.getString(CategoriaProdutoProdutoContract.COL_ID_PRODUTO_RA));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CategoriaProdutoProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CategoriaProdutoProdutoContract.COLUNA_ID_CATEGORIA_PRODUTO_PRODUTO, obj.getString("IdCategoriaProdutoProduto"));
		
		valores.put(CategoriaProdutoProdutoContract.COLUNA_DATA_INCLUSAO, obj.getLong("DataInclusao"));
		
		valores.put(CategoriaProdutoProdutoContract.COLUNA_ID_CATEGORIA_PRODUTO_RA, obj.getString("IdCategoriaProdutoRa"));
		
		valores.put(CategoriaProdutoProdutoContract.COLUNA_ID_PRODUTO_RA, obj.getString("IdProdutoRa"));
