...
public class VendaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(VendaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(VendaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_venda=" + valor.getAsString("id_venda");
                contexto.getContentResolver().update(VendaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdVenda",c.getString(VendaContract.COL_ID_VENDA));
		
			
		
		json.put("Data",c.getString(VendaContract.COL_DATA));
		
			
		
		json.put("ValorTotal",c.getString(VendaContract.COL_VALOR_TOTAL));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("VendaFechada",c.getInt(VendaContract.COL_VENDA_FECHADA)==1);
		
			
		
		json.put("IdClienteFp",c.getString(VendaContract.COL_ID_CLIENTE_FP));
		
			
		
		json.put("IdUsuarioS",c.getString(VendaContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(VendaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(VendaContract.COLUNA_ID_VENDA, obj.getString("IdVenda"));
		
		valores.put(VendaContract.COLUNA_DATA, obj.getLong("Data"));
		
		valores.put(VendaContract.COLUNA_VALOR_TOTAL, obj.getString("ValorTotal"));
			
		int valVendaFechada = 0;
		if ("True".equals(obj.getString("VendaFechada"))) {
			valVendaFechada = 1;
		}
		valores.put(VendaContract.COLUNA_VENDA_FECHADA, valVendaFechada);
		
		valores.put(VendaContract.COLUNA_ID_CLIENTE_FP, obj.getString("IdClienteFp"));
		
		valores.put(VendaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
