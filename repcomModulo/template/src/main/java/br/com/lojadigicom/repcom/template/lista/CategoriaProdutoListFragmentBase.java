...
public abstract class CategoriaProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private CategoriaProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 398;
	...
	public CategoriaProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, CategoriaProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CATEGORIA_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(CategoriaProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new CategoriaProdutoListAdapterBase.CategoriaProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, CategoriaProdutoListViewHolderBase vh) {
    ...
    protected abstract CategoriaProdutoListAdapterBase getListAdapter(FragmentActivity activity, CategoriaProdutoListAdapterBase.CategoriaProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(CategoriaProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CATEGORIA_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CATEGORIA_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<CategoriaProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista CategoriaProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<CategoriaProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return CategoriaProdutoContract.getMontador();
    }

    protected final List<CategoriaProduto> converteLista(Cursor data) {
