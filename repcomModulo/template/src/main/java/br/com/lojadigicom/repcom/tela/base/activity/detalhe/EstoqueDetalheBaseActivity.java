...
public abstract class EstoqueDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 470 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(EstoqueContract.COM_COMPLEMENTO).build();
        uri = EstoqueContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return EstoqueContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
