...
public abstract class PagamentoFornecedorDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 405 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PagamentoFornecedorContract.COM_COMPLEMENTO).build();
        uri = PagamentoFornecedorContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PagamentoFornecedorContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
