...
public abstract class MesAnoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{
    ...
    private static final int FORECAST_LOADER = 773;
	...
	private final void onItemSelected(int idItemLista, MesAnoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.MESANO_ID,idItemLista);
        if (idItemLista==0) {
	...
	/*
 	public void registraPosicao(MesAnoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.MESANO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.MESANO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
