...
public abstract class ProdutoVendaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ProdutoVendaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 404;
	...
	public ProdutoVendaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ProdutoVendaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PRODUTO_VENDA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ProdutoVendaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ProdutoVendaListAdapterBase.ProdutoVendaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ProdutoVendaListViewHolderBase vh) {
    ...
    protected abstract ProdutoVendaListAdapterBase getListAdapter(FragmentActivity activity, ProdutoVendaListAdapterBase.ProdutoVendaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ProdutoVendaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRODUTO_VENDA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRODUTO_VENDA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ProdutoVenda> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ProdutoVenda " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ProdutoVenda> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ProdutoVendaContract.getMontador();
    }

    protected final List<ProdutoVenda> converteLista(Cursor data) {
