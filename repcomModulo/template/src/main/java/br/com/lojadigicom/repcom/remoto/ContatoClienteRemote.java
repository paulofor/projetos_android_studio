...
public class ContatoClienteRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ContatoClienteContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ContatoClienteContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_contato_cliente=" + valor.getAsString("id_contato_cliente");
                contexto.getContentResolver().update(ContatoClienteContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdContatoCliente",c.getString(ContatoClienteContract.COL_ID_CONTATO_CLIENTE));
		
			
		
		json.put("DataContato",c.getString(ContatoClienteContract.COL_DATA_CONTATO));
		
			
		
		json.put("IdClienteRa",c.getString(ContatoClienteContract.COL_ID_CLIENTE_RA));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ContatoClienteContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ContatoClienteContract.COLUNA_ID_CONTATO_CLIENTE, obj.getString("IdContatoCliente"));
		
		valores.put(ContatoClienteContract.COLUNA_DATA_CONTATO, obj.getLong("DataContato"));
		
		valores.put(ContatoClienteContract.COLUNA_ID_CLIENTE_RA, obj.getString("IdClienteRa"));
