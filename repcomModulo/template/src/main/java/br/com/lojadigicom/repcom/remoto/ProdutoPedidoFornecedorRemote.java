...
public class ProdutoPedidoFornecedorRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ProdutoPedidoFornecedorContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ProdutoPedidoFornecedorContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_produto_pedido_fornecedor=" + valor.getAsString("id_produto_pedido_fornecedor");
                contexto.getContentResolver().update(ProdutoPedidoFornecedorContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdProdutoPedidoFornecedor",c.getString(ProdutoPedidoFornecedorContract.COL_ID_PRODUTO_PEDIDO_FORNECEDOR));
		
			
		
		json.put("IdPedidoFornecedorA",c.getString(ProdutoPedidoFornecedorContract.COL_ID_PEDIDO_FORNECEDOR_A));
		
			
		
		json.put("IdProdutoA",c.getString(ProdutoPedidoFornecedorContract.COL_ID_PRODUTO_A));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ProdutoPedidoFornecedorContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ProdutoPedidoFornecedorContract.COLUNA_ID_PRODUTO_PEDIDO_FORNECEDOR, obj.getString("IdProdutoPedidoFornecedor"));
		
		valores.put(ProdutoPedidoFornecedorContract.COLUNA_ID_PEDIDO_FORNECEDOR_A, obj.getString("IdPedidoFornecedorA"));
		
		valores.put(ProdutoPedidoFornecedorContract.COLUNA_ID_PRODUTO_A, obj.getString("IdProdutoA"));
