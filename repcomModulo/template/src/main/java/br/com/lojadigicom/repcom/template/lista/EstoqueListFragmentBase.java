...
public abstract class EstoqueListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private EstoqueListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 470;
	...
	public EstoqueListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, EstoqueListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.ESTOQUE_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(EstoqueListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new EstoqueListAdapterBase.EstoqueListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, EstoqueListViewHolderBase vh) {
    ...
    protected abstract EstoqueListAdapterBase getListAdapter(FragmentActivity activity, EstoqueListAdapterBase.EstoqueListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(EstoqueListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.ESTOQUE_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.ESTOQUE_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<Estoque> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista Estoque " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<Estoque> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return EstoqueContract.getMontador();
    }

    protected final List<Estoque> converteLista(Cursor data) {
