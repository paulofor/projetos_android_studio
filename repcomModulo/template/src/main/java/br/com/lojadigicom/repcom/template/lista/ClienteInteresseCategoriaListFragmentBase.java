...
public abstract class ClienteInteresseCategoriaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ClienteInteresseCategoriaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 408;
	...
	public ClienteInteresseCategoriaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ClienteInteresseCategoriaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CLIENTE_INTERESSE_CATEGORIA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ClienteInteresseCategoriaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ClienteInteresseCategoriaListAdapterBase.ClienteInteresseCategoriaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ClienteInteresseCategoriaListViewHolderBase vh) {
    ...
    protected abstract ClienteInteresseCategoriaListAdapterBase getListAdapter(FragmentActivity activity, ClienteInteresseCategoriaListAdapterBase.ClienteInteresseCategoriaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ClienteInteresseCategoriaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CLIENTE_INTERESSE_CATEGORIA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CLIENTE_INTERESSE_CATEGORIA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ClienteInteresseCategoria> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ClienteInteresseCategoria " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ClienteInteresseCategoria> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ClienteInteresseCategoriaContract.getMontador();
    }

    protected final List<ClienteInteresseCategoria> converteLista(Cursor data) {
	...
	protected long mIdCliente;
	public final void setIdCliente(long id) {
		mIdCliente = id;
	}
