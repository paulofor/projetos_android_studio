...
public class CategoriaProdutoRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CategoriaProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CategoriaProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_categoria_produto=" + valor.getAsString("id_categoria_produto");
                contexto.getContentResolver().update(CategoriaProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCategoriaProduto",c.getString(CategoriaProdutoContract.COL_ID_CATEGORIA_PRODUTO));
		
			
		
		json.put("Nome",c.getString(CategoriaProdutoContract.COL_NOME));
		
			
		
		json.put("Url",c.getString(CategoriaProdutoContract.COL_URL));
		
			
		
		json.put("CodigoLineId",c.getString(CategoriaProdutoContract.COL_CODIGO_LINE_ID));
		
			
		
		json.put("DataInclusao",c.getString(CategoriaProdutoContract.COL_DATA_INCLUSAO));
		
			
		
		json.put("IdCategoriaProdutoP",c.getString(CategoriaProdutoContract.COL_ID_CATEGORIA_PRODUTO_P));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CategoriaProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CategoriaProdutoContract.COLUNA_ID_CATEGORIA_PRODUTO, obj.getString("IdCategoriaProduto"));
		
		valores.put(CategoriaProdutoContract.COLUNA_NOME, obj.getString("Nome"));
		
		valores.put(CategoriaProdutoContract.COLUNA_URL, obj.getString("Url"));
		
		valores.put(CategoriaProdutoContract.COLUNA_CODIGO_LINE_ID, obj.getString("CodigoLineId"));
		
		valores.put(CategoriaProdutoContract.COLUNA_DATA_INCLUSAO, obj.getLong("DataInclusao"));
		
		valores.put(CategoriaProdutoContract.COLUNA_ID_CATEGORIA_PRODUTO_P, obj.getString("IdCategoriaProdutoP"));
