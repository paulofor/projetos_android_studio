...
public class ErroExceptionRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ErroExceptionContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ErroExceptionContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_erro_exception=" + valor.getAsString("id_erro_exception");
                contexto.getContentResolver().update(ErroExceptionContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdErroException",c.getString(ErroExceptionContract.COL_ID_ERRO_EXCEPTION));
		
			
		
		json.put("Stack",c.getString(ErroExceptionContract.COL_STACK));
		
			
		
		json.put("Aplicacao",c.getString(ErroExceptionContract.COL_APLICACAO));
		
			
		
		json.put("IdUsuarioS",c.getString(ErroExceptionContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ErroExceptionContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ErroExceptionContract.COLUNA_ID_ERRO_EXCEPTION, obj.getString("IdErroException"));
		
		valores.put(ErroExceptionContract.COLUNA_STACK, obj.getString("Stack"));
		
		valores.put(ErroExceptionContract.COLUNA_APLICACAO, obj.getString("Aplicacao"));
		
		valores.put(ErroExceptionContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
