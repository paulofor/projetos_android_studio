...
public abstract class PedidoFornecedorListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PedidoFornecedorListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 399;
	...
	public PedidoFornecedorListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, PedidoFornecedorListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PEDIDO_FORNECEDOR_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(PedidoFornecedorListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new PedidoFornecedorListAdapterBase.PedidoFornecedorListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PedidoFornecedorListViewHolderBase vh) {
    ...
    protected abstract PedidoFornecedorListAdapterBase getListAdapter(FragmentActivity activity, PedidoFornecedorListAdapterBase.PedidoFornecedorListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PedidoFornecedorListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PEDIDO_FORNECEDOR_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PEDIDO_FORNECEDOR_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PedidoFornecedor> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PedidoFornecedor " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<PedidoFornecedor> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PedidoFornecedorContract.getMontador();
    }

    protected final List<PedidoFornecedor> converteLista(Cursor data) {
