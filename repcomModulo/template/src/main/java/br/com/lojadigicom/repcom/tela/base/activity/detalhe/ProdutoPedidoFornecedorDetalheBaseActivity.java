...
public abstract class ProdutoPedidoFornecedorDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 403 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ProdutoPedidoFornecedorContract.COM_COMPLEMENTO).build();
        uri = ProdutoPedidoFornecedorContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ProdutoPedidoFornecedorContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
