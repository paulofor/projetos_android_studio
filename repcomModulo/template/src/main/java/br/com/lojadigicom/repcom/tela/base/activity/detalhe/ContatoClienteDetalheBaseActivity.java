...
public abstract class ContatoClienteDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 401 * 10;
    private long idContatoCliente = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idContatoCliente = extra.getInt(Constantes.CONTATO_CLIENTE_ID);
        } catch (Exception e) {
            idContatoCliente = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idContatoCliente==0) {
            throw new RuntimeException("Nao recebeu o IdContatoCliente");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idContatoCliente);
        edt.commit();
        ...
        complementaOnCreate(idContatoCliente);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ContatoClienteContract.buildContatoClienteUri(idContatoCliente);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ContatoClienteContract.COM_COMPLEMENTO).build();
        uri = ContatoClienteContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ContatoClienteContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ContatoClienteContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        ContatoCliente item = (ContatoCliente) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(ContatoCliente item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idContatoCliente);
