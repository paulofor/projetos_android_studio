...
public abstract class ParcelaVendaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 406 * 10;
    private long idParcelaVenda = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idParcelaVenda = extra.getInt(Constantes.PARCELA_VENDA_ID);
        } catch (Exception e) {
            idParcelaVenda = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idParcelaVenda==0) {
            throw new RuntimeException("Nao recebeu o IdParcelaVenda");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idParcelaVenda);
        edt.commit();
        ...
        complementaOnCreate(idParcelaVenda);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ParcelaVendaContract.buildParcelaVendaUri(idParcelaVenda);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ParcelaVendaContract.COM_COMPLEMENTO).build();
        uri = ParcelaVendaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ParcelaVendaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ParcelaVendaContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        ParcelaVenda item = (ParcelaVenda) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(ParcelaVenda item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idParcelaVenda);
