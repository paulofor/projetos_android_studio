...
public abstract class UsuarioDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 471 * 10;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idUsuario = extra.getInt(Constantes.USUARIO_ID);
        } catch (Exception e) {
            idUsuario = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idUsuario==0) {
            throw new RuntimeException("Nao recebeu o IdUsuario");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idUsuario);
        edt.commit();
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = UsuarioContract.buildUsuarioUri(idUsuario);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(UsuarioContract.COM_COMPLEMENTO).build();
        uri = UsuarioContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return UsuarioContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = UsuarioContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        Usuario item = (Usuario) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(Usuario item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idUsuario);
