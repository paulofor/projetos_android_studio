...
public abstract class ErroExceptionDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 854 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ErroExceptionContract.COM_COMPLEMENTO).build();
        uri = ErroExceptionContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ErroExceptionContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
