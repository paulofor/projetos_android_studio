...
public abstract class CategoriaProdutoProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 678 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(CategoriaProdutoProdutoContract.COM_COMPLEMENTO).build();
        uri = CategoriaProdutoProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return CategoriaProdutoProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
