...
public abstract class PedidoFornecedorDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 399 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PedidoFornecedorContract.COM_COMPLEMENTO).build();
        uri = PedidoFornecedorContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PedidoFornecedorContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
