...
public abstract class PagamentoFornecedorListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PagamentoFornecedorListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 405;
	...
	public PagamentoFornecedorListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, PagamentoFornecedorListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PAGAMENTO_FORNECEDOR_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(PagamentoFornecedorListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new PagamentoFornecedorListAdapterBase.PagamentoFornecedorListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PagamentoFornecedorListViewHolderBase vh) {
    ...
    protected abstract PagamentoFornecedorListAdapterBase getListAdapter(FragmentActivity activity, PagamentoFornecedorListAdapterBase.PagamentoFornecedorListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PagamentoFornecedorListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PAGAMENTO_FORNECEDOR_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.repcom", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PAGAMENTO_FORNECEDOR_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PagamentoFornecedor> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PagamentoFornecedor " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<PagamentoFornecedor> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PagamentoFornecedorContract.getMontador();
    }

    protected final List<PagamentoFornecedor> converteLista(Cursor data) {
