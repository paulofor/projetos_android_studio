...
public abstract class ClienteInteresseCategoriaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 408 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ClienteInteresseCategoriaContract.COM_COMPLEMENTO).build();
        uri = ClienteInteresseCategoriaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ClienteInteresseCategoriaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
