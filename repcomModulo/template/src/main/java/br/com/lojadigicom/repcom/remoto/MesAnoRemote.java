...
public class MesAnoRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(MesAnoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(MesAnoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_mes_ano=" + valor.getAsString("id_mes_ano");
                contexto.getContentResolver().update(MesAnoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdMesAno",c.getString(MesAnoContract.COL_ID_MES_ANO));
		
			
		
		json.put("Mes",c.getString(MesAnoContract.COL_MES));
		
			
		
		json.put("Ano",c.getString(MesAnoContract.COL_ANO));
		
			
		
		json.put("DescricaoTela",c.getString(MesAnoContract.COL_DESCRICAO_TELA));
		
			
		
		json.put("DataReferencia",c.getString(MesAnoContract.COL_DATA_REFERENCIA));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(MesAnoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(MesAnoContract.COLUNA_ID_MES_ANO, obj.getString("IdMesAno"));
		
		valores.put(MesAnoContract.COLUNA_MES, obj.getString("Mes"));
		
		valores.put(MesAnoContract.COLUNA_ANO, obj.getString("Ano"));
		
		valores.put(MesAnoContract.COLUNA_DESCRICAO_TELA, obj.getString("DescricaoTela"));
		
		valores.put(MesAnoContract.COLUNA_DATA_REFERENCIA, obj.getLong("DataReferencia"));
