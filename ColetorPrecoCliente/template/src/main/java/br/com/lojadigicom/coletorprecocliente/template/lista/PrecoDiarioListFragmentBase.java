...
public abstract class PrecoDiarioListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PrecoDiarioListAdapterBase mAdapter;
    ...
    public interface Callback {
        ...
        public void onItemSelected(int idItemLista, PrecoDiarioListViewHolderBase vh);
    }

    public PrecoDiarioListFragmentBase() {
    }
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(getActivity(), new PrecoDiarioListAdapterBase.PrecoDiarioListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PrecoDiarioListViewHolderBase vh) {
    ...
    protected abstract PrecoDiarioListAdapterBase getListAdapter(FragmentActivity activity, PrecoDiarioListAdapterBase.PrecoDiarioListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PrecoDiarioListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRECO_DIARIO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRECO_DIARIO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PrecoDiario> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PrecoDiario " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PrecoDiarioContract.getMontador();
    }

    protected final List<PrecoDiario> converteLista(Cursor data) {
