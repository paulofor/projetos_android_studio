...
public abstract class UsuarioServicoWsJsonBase 
		extends  UsuarioServico 
		implements ServicoRemoto<Usuario>{

	private List<Usuario> listaSaida = null;
	...
	public UsuarioServicoWsJsonBase() {
		setDataSource(new DataSourceRemoto());
	}
	
	
	@Override
	public void listaSincronizadaAlteracao(List<Usuario> listaSinc, Context contexto) throws JSONException {
		Map<String,String> params = new HashMap<String,String>();
		String url = getUrl("ListaSincronizadaAlteracao");
		String saida = " [ ";
		boolean primeiro = true;
		for (Usuario item:listaSinc) {
			if (primeiro) {
				primeiro = false;
			} else {
				saida += " , ";
		...
		DCLog.d(DCLog.SINCRONIZACAO_JSON,this,"Enviando...lista<Usuario>:" + saida);
	...
	private static final String URL_GET_POSTS_RECENT = 
		"http://www.lojadigicom.com.br/treinoacademia/page/UsuarioPage.aspx";
	...
	private final Handler handler = new Handler() {
		public void handleMessage(final Message msg) {
			...
			listaSaida = new ArrayList<Usuario>();
			try {
				json = new JSONObject(bundleResult);
				JSONArray lista = json.getJSONArray("Usuario");
				int tam = lista.length();
				JSONObject item = null;
				String nome = null;
				for (int i=0;i<lista.length();i++) {
					item = (JSONObject) lista.get(i);
					listaSaida.add(new Usuario(item));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	*/
	...
	public Usuario ObtemNoAparelho(DigicomContexto ctx){
		String resultado = HTTPRequestHelper.getHttpResponse(getUrl("ObtemNoAparelho"));
	...
	private int processaResultado(String resultado, DaoSincronismo daoSinc) {
		...
		try {
			...
			for (int i=0;i<lista.length();i++) {
				item = (JSONObject) lista.get(i);
				obj = FabricaVo.criaUsuario(item);
				daoSinc.insere(obj);
			}
			lista = null;
		} catch (Exception e) {
	...
	@Deprecated
	private List<Usuario> processaResultado(String resultado) {
		int inicio = resultado.indexOf('{');
		resultado = resultado.substring(inicio);
		List<Usuario> listaSaida = new LinkedList<Usuario>();
		...
		try {
			...
			for (int i=0;i<lista.length();i++) {
				item = (JSONObject) lista.get(i);
				//corrente = FabricaVo.criaUsuario(item);
				//servicoLocal.insertLocal(corrente);
				listaSaida.add(FabricaVo.criaUsuario(item));
			}
			lista = null;			
		} catch (Exception e) {
			//Log.e("UsuarioServicoWsJson", e.getMessage() + " [ " + ultimaUrl + " ]");
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
			TrataErro.getInstancia().setErro(e);
		}
	...
	private Usuario processaResultadoObjeto(String resultado) {
		int inicio = resultado.indexOf('{');
		resultado = resultado.substring(inicio);
		Usuario saida = null;
		...
		try {
			...
			String nome = null;
			saida = FabricaVo.criaUsuario(item);
	...
	public final void alteraParaSincronizacao(Usuario item) {
		throw new UnsupportedOperationException();
	}
	public final void insereParaSincronizacao(Usuario item) {
		throw new UnsupportedOperationException();
	}
	public final void excluiParaSincronizacao(Usuario item) {
		throw new UnsupportedOperationException();
	}
	public final void criaTabelaSincronizacao() {
	}
	
	public final Usuario getCorrente() {
		throw new UnsupportedOperationException();
	}
	...
	public final Usuario ultimoInicializado() {
		throw new UnsupportedOperationException();
	}
	...
	*/
	...
	@Deprecated
	public List<Usuario> listaSincronizadaUsuarioV2(Context contexto) {
	...
	private List<Usuario> processaResultadoLista(List<String> lista) {
		List<Usuario> listaSaida = new LinkedList<Usuario>();
		try {
			for (String palavra : lista) {
				JSONObject json = new JSONObject(palavra);
				Usuario item = FabricaVo.criaUsuario(json);
				listaSaida.add(item);
			}
		} catch (Exception e) {
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
	...
	private int processaResultadoLista(List<String> lista , DaoSincronismo daoSinc) {
		...
		try {
			...
			for (String palavra : lista) {
				JSONObject json = new JSONObject(palavra);
				item = FabricaVo.criaUsuario(json);
				daoSinc.insere(item);
			}
		} catch (Exception e) {
			DCLog.e(DCLog.SINCRONIZACAO_JSON, this, e);
