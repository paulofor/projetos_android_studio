...
public abstract class UsuarioQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected void setOnOk(View tela) {
		View btn = (View) tela.findViewById(R.id.btnOkTrataUsuario);
		// Verificando se existe o bot?o de ok na tela.
		if (btn==null) {
			throw new RuntimeException("R.id.btnOkTrataUsuario n?o encontrado em " + this.getRecurso().getNome());
		}
	...
	public boolean posInsereRetornaTela(Usuario valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(Usuario valor) {
		return true;
	}
	...
	public final void extraiBundle() {
		this.item = (Usuario) this.getBundle().getObjeto(Constantes.CHAVE_USUARIO);
