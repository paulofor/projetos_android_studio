...
public abstract class CargaPlanejadaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {

	
	private CargaPlanejada item = null;
	private boolean insercao = false;
	private boolean alteracao = false;
	//protected Activity activity = null;
	
	// Para manipular apenas os dados relacionados
	protected CargaPlanejadaAgregadoI getItem() {
		return (CargaPlanejadaAgregadoI) item;
	}
	...
	protected void setOnOk(View tela) {
		View btn = (View) tela.findViewById(R.id.btnOkTrataCargaPlanejada);
		// Verificando se existe o bot?o de ok na tela.
		if (btn==null) {
			throw new RuntimeException("R.id.btnOkTrataCargaPlanejada n?o encontrado em " + this.getRecurso().getNome());
		}
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para CargaPlanejadaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em CargaPlanejadaQuadroTrataBase");
					}
	...
	public boolean posInsereRetornaTela(CargaPlanejada valor) {
		return true;
	}
	public boolean posAlteraRetornaTela(CargaPlanejada valor) {
		return true;
	}

	protected abstract ResultadoValidacao validaCamposTela();
	//	throw new RuntimeException("Implementar validaCamposTela de CargaPlanejadaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(CargaPlanejada valor) {
		CargaPlanejadaServico exibicaoSrv = FabricaServico.getInstancia().getCargaPlanejadaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_carga_planejada, null);
	...
	// outro quadro
	protected final void insereItem(CargaPlanejada valor) {
		CargaPlanejadaServico exibicaoSrv = FabricaServico.getInstancia().getCargaPlanejadaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_carga_planejada, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(CargaPlanejada vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em CargaPlanejadaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final CargaPlanejada vo)  {
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_carga_planejada,"R.layout.trata_carga_planejada");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaCargaPlanejada();
			item = servico.inicializaItemTela(item, getContext());
		}
			
		if (this.itemSerie!=null) {
	...
	protected Spinner getSpinnerItemSerie(int codigo, String nome) {
		ItemSerieServico srv = FabricaServico.getInstancia().getItemSerieServico(FabricaServico.TIPO_SQLITE);
		List<ItemSerie> listaTela = srv.getAllTela(getContext().getContext());
		Spinner spn = this.getSpinner(codigo, nome, listaTela, this.getContext().getContext()); 
		return spn;
	}
	/*
	protected Spinner carregaSpinnerItemSerie(Spinner spn) {
		ItemSerieServico srv = FabricaServico.getInstancia().getItemSerieServico(FabricaServico.TIPO_SQLITE);
		List<ItemSerie> listaTela = srv.getAllTela(getContext().getContext());
		ArrayAdapter<ItemSerie> dataAdapter = new ArrayAdapter<ItemSerie>(getContext().getContext(), android.R.layout.simple_spinner_item, listaTela);
	...
	*/
	...
	public final void extraiBundle() {
		this.item = (CargaPlanejada) this.getBundle().getObjeto(Constantes.CHAVE_CARGA_PLANEJADA);
		...
		this.itemSerie = (ItemSerie) this.getBundle().getObjetoPermiteNull(Constantes.CHAVE_ITEM_SERIE);
		existeItem = existeItem || (this.itemSerie!=null);
