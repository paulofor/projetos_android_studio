...
import treinoacademia.view.adapter.ItemSerieListAdapter;
...
public abstract class ItemSerieListaEdicaoBase extends BaseListFragment implements IQuadroListaEdicao  
	{

	public final static String ITEM_CLICK = "ItemSerieItemClick";

	private ItemSerieListEdicaoAdapter adapt = null;
	private List<ItemSerie> lista = null;
	...
	protected final void inicializaItensTelaBase() {
		//btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaItemSerie);
		logObjetoInterno();
	}
	@Override
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_item_serie,"R.layout.lista_item_serie");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		//ItemSerie nova = servico.inicializaItemTela(getContext());
		ItemSerie nova = FabricaVo.criaItemSerie();
		nova = insereObjetoPrincipal(nova);
		nova = (ItemSerie) ((ServicoLocal)servico).atribuiUsuario(nova);
		nova = servico.inicializaItemTela(nova,getContext());
		BundleFragment bundle = new BundleFragment();
		bundle.setObjeto(Constantes.CHAVE_ITEM_SERIE, nova);
	...
	/*
	protected ItemSerie criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em ItemSerieQuadroLista retornando new ItemSerie com inicializa??o de listas internas se necessario");
	...
	*/
	...
	protected IFragmentEdicao criaQuadroTrata(BundleFragment bundle) {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em ItemSerieListaEdicao retornando new ItemSerieQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected void preencheLista() {
        ...
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<ItemSerie> -> " + lista.size() + " itens");
	...
	private ItemSerieListEdicaoAdapter getAdapter(List<ItemSerie> lista,DigicomContexto dContexto) {
		return new ItemSerieListEdicaoAdapter(lista,this,dContexto.getContext());
	}
	...
	private ItemSerie insereObjetoPrincipal(ItemSerie item) {
	...
	protected List<ItemSerie> getListaCorrente(Context contexto, ItemSerieServico servico) {
		...
		if (serieTreino!=null) {
			listaSaida = servico.getPorPertencenteASerieTreino(contexto, serieTreino.getId());
		}
