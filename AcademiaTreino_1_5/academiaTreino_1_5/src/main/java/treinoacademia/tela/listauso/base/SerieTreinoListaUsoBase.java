...
import treinoacademia.view.adapter.listauso.SerieTreinoListUsoAdapter;
...
public abstract class SerieTreinoListaUsoBase extends BaseListFragment implements IQuadroLista {

	public final static String ITEM_CLICK = "SerieTreinoItemClick";

	private SerieTreinoListUsoAdapterBase adapt = null;
	...
	protected void atualizaLista(List<SerieTreino> listaNova) {
	...
	*/
	...
	private void preencheLista() {
        ...
        List<SerieTreino> lista = getListaCorrente(dContexto.getContext(),servico);
        // Pode ser necessario um adapter customizado (diferenciar o editar e usar)
        adapt =  getAdapter(lista, dContexto);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "getListaCorrente : List<SerieTreino> -> " + lista.size() + " itens (Adapter:" + adapt.getClass().getName() + ")");
        adapt.setRaiz(this);
        setListAdapter(adapt);
	}
	
	protected SerieTreinoListUsoAdapterBase getAdapter(List<SerieTreino> lista,DigicomContexto dContexto) {
		return new SerieTreinoListUsoAdapter(lista,dContexto);
	}
	
	protected List<SerieTreino> getListaCorrente(Context contexto,SerieTreinoServico servico) {
