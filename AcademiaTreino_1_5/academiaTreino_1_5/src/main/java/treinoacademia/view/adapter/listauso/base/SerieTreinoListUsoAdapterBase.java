...
public abstract class SerieTreinoListUsoAdapterBase extends DCBaseAdapter {
	...
	public SerieTreinoListUsoAdapterBase(List<SerieTreino> lista, DigicomContexto context ) {
		this.context = context;
		this.lista = lista;
		setEdicao(); // Forcando para fazer edicao - alterar se for necessario depois.
	}
	
	public void setListaNova(List<SerieTreino> listaNova) {
	...
	protected void montaItemLista(int position, SerieTreino item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaIItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_serie_treino");
	}
	protected ResourceObj getLayoutItem() {
		return new ResourceObj(R.layout.item_serie_treino,"R.layout.item_serie_treino");
	}
