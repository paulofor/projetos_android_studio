...
public abstract class SerieTreinoListEdicaoAdapterBase extends DCBaseAdapter implements OnTouchListener, OnLongClickListener{
	
	private Context context;
	private List<SerieTreino> lista;
	...
	public SerieTreinoListEdicaoAdapterBase(List<SerieTreino> lista, IQuadroListaEdicao origem, Context context ) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		SerieTreino item = lista.get(position);
		
		View saida = LayoutInflater.from(context).inflate(getLayout(), null);
		if (!(saida instanceof IItemLista))
			throw new RuntimeException("Alterar layout raiz de R.layout.item_serie_treino para br.com.digicom.layout.IItemLista");
	...
	protected int getLayout() {
		return R.layout.item_serie_treino;
	}
	
	protected void montaItemLista(int position, SerieTreino item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_serie_treino");
	}
	
	@Override
	public boolean onTouch(View view, MotionEvent avent) {
	 	ItemLista objeto = (ItemLista) view;
		SerieTreino item = (SerieTreino) objeto.getItem();
