...
import treinoacademia.tela.quadro.base.CargaPlanejadaQuadroListaBase;

public abstract class CargaPlanejadaListAdapterBase extends DCBaseAdapter implements OnTouchListener, OnLongClickListener{
	
	private Context context;
	private List<CargaPlanejada> lista;
	...
	public CargaPlanejadaListAdapterBase(List<CargaPlanejada> lista, Context context ) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		CargaPlanejada item = lista.get(position);
		
		View saida = LayoutInflater.from(context).inflate(R.layout.item_carga_planejada, null);
		if (!(saida instanceof ItemLista))
			throw new RuntimeException("Alterar layout raiz de R.layout.item_carga_planejada para br.com.digicom.layout.IItemLista");
	...
	protected void montaItemLista(int posicao, CargaPlanejada item, View saida) {
		throw new UnsupportedOperationException("Fazer override de montaItemLista em "  + this.getClass().getSimpleName() + " ou verificar se nao esta sendo chamada via super. Arquivo de visao padrao eh: R.layout.item_carga_planejada");
	}
	
	@Override
	public boolean onTouch(View view, MotionEvent avent) {
	 	ItemLista objeto = (ItemLista) view;
		CargaPlanejada item = (CargaPlanejada) objeto.getItem();
