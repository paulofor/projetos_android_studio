...
public class SerieTreinoMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (SerieTreino) entrada;item.setIdSerieTreino(getInt(c,pos++));
		item.setQtdeExecucao(getInt(c,pos++));
		item.setAtiva(getLogic(c,pos++));
		item.setDataCriacao(getTimestampData(c,pos++));
		item.setDataUltimaExecucao(getTimestampData(c,pos++));
