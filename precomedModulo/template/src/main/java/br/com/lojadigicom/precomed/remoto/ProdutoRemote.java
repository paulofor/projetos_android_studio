...
public class ProdutoRemote {
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
        ...
        JSONObject json = new JSONObject();
			...
			json.put("UrlOrigem",c.getString(ProdutoContract.COL_URL_ORIGEM));
			json.put("ImagemLocal",c.getString(ProdutoContract.COL_IMAGEM_LOCAL));
			...
			json.put("PosicaoProduto",c.getString(ProdutoContract.COL_POSICAO_PRODUTO));
			json.put("Imagem",c.getString(ProdutoContract.COL_IMAGEM));
			json.put("CodigoMs",c.getString(ProdutoContract.COL_CODIGO_MS));
			json.put("PrincipioAtivo",c.getString(ProdutoContract.COL_PRINCIPIO_ATIVO));
			json.put("PossuiEstoque",c.getString(ProdutoContract.COL_POSSUI_ESTOQUE));
			json.put("IdLojaVirtualLe",c.getString(ProdutoContract.COL_ID_LOJA_VIRTUAL_LE));
			json.put("IdMarcaP",c.getString(ProdutoContract.COL_ID_MARCA_P));
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ProdutoContract.COLUNA_URL_ORIGEM, obj.getString("UrlOrigem"));
		
		valores.put(ProdutoContract.COLUNA_IMAGEM_LOCAL, obj.getString("ImagemLocal"));
		
		valores.put(ProdutoContract.COLUNA_DATA_INCLUSAO, obj.getLong("DataInclusao"));
		
		valores.put(ProdutoContract.COLUNA_URL, obj.getString("Url"));
		
		valores.put(ProdutoContract.COLUNA_NOME, obj.getString("Nome"));
		
		valores.put(ProdutoContract.COLUNA_POSICAO_PRODUTO, obj.getString("PosicaoProduto"));
		
		valores.put(ProdutoContract.COLUNA_IMAGEM, obj.getString("Imagem"));
		
		valores.put(ProdutoContract.COLUNA_CODIGO_MS, obj.getString("CodigoMs"));
		
		valores.put(ProdutoContract.COLUNA_PRINCIPIO_ATIVO, obj.getString("PrincipioAtivo"));
			
		int valPossuiEstoque = 0;
		if ("True".equals(obj.getString("PossuiEstoque"))) {
			valPossuiEstoque = 1;
		}
		valores.put(ProdutoContract.COLUNA_POSSUI_ESTOQUE, valPossuiEstoque);
		
		valores.put(ProdutoContract.COLUNA_ID_LOJA_VIRTUAL_LE, obj.getString("IdLojaVirtualLe"));
		
		valores.put(ProdutoContract.COLUNA_ID_MARCA_P, obj.getString("IdMarcaP"));
