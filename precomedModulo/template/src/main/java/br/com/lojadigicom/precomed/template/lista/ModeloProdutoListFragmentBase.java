...
public abstract class ModeloProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ModeloProdutoListAdapterBase mAdapter;
	...
	public abstract void onItemSelected(int idItemLista, ModeloProdutoListViewHolderBase vh);
	

    public ModeloProdutoListFragmentBase() {
    }
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ModeloProdutoListAdapterBase.ModeloProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ModeloProdutoListViewHolderBase vh) {
    ...
    protected abstract ModeloProdutoListAdapterBase getListAdapter(FragmentActivity activity, ModeloProdutoListAdapterBase.ModeloProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ModeloProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.MODELO_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.MODELO_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ModeloProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ModeloProduto " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ModeloProdutoContract.getMontador();
    }

    protected final List<ModeloProduto> converteLista(Cursor data) {
