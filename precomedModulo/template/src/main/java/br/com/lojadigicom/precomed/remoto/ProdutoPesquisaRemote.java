...
public class ProdutoPesquisaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ProdutoPesquisaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ProdutoPesquisaContract.getContentUri(), vetor);
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
        ...
        JSONObject json = new JSONObject();
        	json.put("IdProdutoPesquisa",c.getString(ProdutoPesquisaContract.COL_ID_PRODUTO_PESQUISA));
			json.put("DataInclusao",c.getString(ProdutoPesquisaContract.COL_DATA_INCLUSAO));
			json.put("Ativo",c.getString(ProdutoPesquisaContract.COL_ATIVO));
			json.put("NomeProdutoPesquisa",c.getString(ProdutoPesquisaContract.COL_NOME_PRODUTO_PESQUISA));
			json.put("IdUsuarioS",c.getString(ProdutoPesquisaContract.COL_ID_USUARIO_S));
			json.put("IdModeloProdutoRa",c.getString(ProdutoPesquisaContract.COL_ID_MODELO_PRODUTO_RA));
		
        return json;
    }
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        JSONObject json = JSonAtributos(c);
        json.put("OperacaoSinc",c.getString(ProdutoPesquisaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ProdutoPesquisaContract.COLUNA_ID_PRODUTO_PESQUISA, obj.getString("IdProdutoPesquisa"));
		
		valores.put(ProdutoPesquisaContract.COLUNA_DATA_INCLUSAO, obj.getLong("DataInclusao"));
			
		int valAtivo = 0;
		if ("True".equals(obj.getString("Ativo"))) {
			valAtivo = 1;
		}
		valores.put(ProdutoPesquisaContract.COLUNA_ATIVO, valAtivo);
		
		valores.put(ProdutoPesquisaContract.COLUNA_NOME_PRODUTO_PESQUISA, obj.getString("NomeProdutoPesquisa"));
		
		valores.put(ProdutoPesquisaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
		
		valores.put(ProdutoPesquisaContract.COLUNA_ID_MODELO_PRODUTO_RA, obj.getString("IdModeloProdutoRa"));
