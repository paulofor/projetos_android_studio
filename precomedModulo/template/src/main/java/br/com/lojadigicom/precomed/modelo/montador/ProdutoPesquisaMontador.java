...
import br.com.lojadigicom.precomed.modelo.ProdutoPesquisaVo;
...
public class ProdutoPesquisaMontador implements MontadorDaoI {

	private ProdutoPesquisa principal = new ProdutoPesquisaVo();
	
	private boolean sinc = false;
	public ProdutoPesquisaMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public ProdutoPesquisaMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c) {
		...
		//objeto = FabricaVo.criaProdutoPesquisa();
		objeto = new ProdutoPesquisaVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		ProdutoPesquisa item = null;
		item = (ProdutoPesquisa) entrada;
		item.setIdProdutoPesquisa(getInt(c,pos++));
		item.setDataInclusao(getTimestampDataHora(c,pos++));
		item.setAtivo(getLogic(c,pos++));
		item.setNomeProdutoPesquisa(getString(c,pos++));
		item.setIdUsuarioS(getInt(c,pos++));
		item.setIdModeloProdutoRa(getInt(c,pos++));
