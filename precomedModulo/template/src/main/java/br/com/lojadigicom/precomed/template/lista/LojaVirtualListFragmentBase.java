...
public abstract class LojaVirtualListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private LojaVirtualListAdapterBase mAdapter;
	...
	public abstract void onItemSelected(int idItemLista, LojaVirtualListViewHolderBase vh);
	

    public LojaVirtualListFragmentBase() {
    }
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new LojaVirtualListAdapterBase.LojaVirtualListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, LojaVirtualListViewHolderBase vh) {
    ...
    protected abstract LojaVirtualListAdapterBase getListAdapter(FragmentActivity activity, LojaVirtualListAdapterBase.LojaVirtualListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(LojaVirtualListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.LOJA_VIRTUAL_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.LOJA_VIRTUAL_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<LojaVirtual> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista LojaVirtual " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return LojaVirtualContract.getMontador();
    }

    protected final List<LojaVirtual> converteLista(Cursor data) {
