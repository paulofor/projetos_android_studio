...
public abstract class OportunidadeDiaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 863 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(OportunidadeDiaContract.COM_COMPLEMENTO).build();
        uri = OportunidadeDiaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return OportunidadeDiaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
