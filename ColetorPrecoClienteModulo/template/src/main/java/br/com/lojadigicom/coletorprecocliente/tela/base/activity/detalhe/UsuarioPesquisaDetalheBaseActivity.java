...
public abstract class UsuarioPesquisaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 879 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(UsuarioPesquisaContract.COM_COMPLEMENTO).build();
        uri = UsuarioPesquisaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return UsuarioPesquisaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
