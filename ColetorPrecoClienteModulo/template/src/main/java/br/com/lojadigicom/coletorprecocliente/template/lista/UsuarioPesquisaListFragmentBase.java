...
public abstract class UsuarioPesquisaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private UsuarioPesquisaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 879;
	...
	public UsuarioPesquisaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, UsuarioPesquisaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.USUARIO_PESQUISA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(UsuarioPesquisaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new UsuarioPesquisaListAdapterBase.UsuarioPesquisaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, UsuarioPesquisaListViewHolderBase vh) {
    ...
    protected abstract UsuarioPesquisaListAdapterBase getListAdapter(FragmentActivity activity, UsuarioPesquisaListAdapterBase.UsuarioPesquisaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(UsuarioPesquisaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.USUARIO_PESQUISA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.USUARIO_PESQUISA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<UsuarioPesquisa> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista UsuarioPesquisa " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<UsuarioPesquisa> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return UsuarioPesquisaContract.getMontador();
    }

    protected final List<UsuarioPesquisa> converteLista(Cursor data) {
