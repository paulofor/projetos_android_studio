...
public class OportunidadeDiaRemote {
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_oportunidade_dia=" + valor.getAsString("id_oportunidade_dia");
                contexto.getContentResolver().update(OportunidadeDiaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(OportunidadeDiaContract.COLUNA_DATA_ULTIMA_PRECO_ANTERIOR, obj.getLong("DataUltimaPrecoAnterior"));
		
		valores.put(OportunidadeDiaContract.COLUNA_DATA_INICIO_PRECO_ATUAL, obj.getLong("DataInicioPrecoAtual"));
