...
public class NaturezaProdutoRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(NaturezaProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(NaturezaProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_natureza_produto=" + valor.getAsString("id_natureza_produto");
                contexto.getContentResolver().update(NaturezaProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdNaturezaProduto",c.getString(NaturezaProdutoContract.COL_ID_NATUREZA_PRODUTO));
		
			
		
		json.put("NomeNaturezaProduto",c.getString(NaturezaProdutoContract.COL_NOME_NATUREZA_PRODUTO));
		
			
		
		json.put("IdAppProdutoAp",c.getString(NaturezaProdutoContract.COL_ID_APP_PRODUTO_AP));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(NaturezaProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(NaturezaProdutoContract.COLUNA_ID_NATUREZA_PRODUTO, obj.getString("IdNaturezaProduto"));
		
		valores.put(NaturezaProdutoContract.COLUNA_NOME_NATUREZA_PRODUTO, obj.getString("NomeNaturezaProduto"));
		
		valores.put(NaturezaProdutoContract.COLUNA_ID_APP_PRODUTO_AP, obj.getString("IdAppProdutoAp"));
		
		
		
		
		valores.put(NaturezaProdutoContract.COLUNA_QTDEOPORTUNIDADEDIA, obj.getString("QtdeOportunidadeDia"));
		
		
			
		int valNaturezaProduto = 0;
		if ("True".equals(obj.getString("Ativo"))) {
			valNaturezaProduto = 1;
		}
		valores.put(NaturezaProdutoContract.COLUNA_ATIVO, valNaturezaProduto);
