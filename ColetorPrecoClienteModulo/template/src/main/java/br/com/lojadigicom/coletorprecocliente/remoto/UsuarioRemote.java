...
public class UsuarioRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(UsuarioContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(UsuarioContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_usuario=" + valor.getAsString("id_usuario");
                contexto.getContentResolver().update(UsuarioContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdUsuario",c.getString(UsuarioContract.COL_ID_USUARIO));
		
			
		
		json.put("NomeUsuario",c.getString(UsuarioContract.COL_NOME_USUARIO));
		
			
		
		json.put("Email",c.getString(UsuarioContract.COL_EMAIL));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Plano01",c.getInt(UsuarioContract.COL_PLANO01)==1);
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Plano02",c.getInt(UsuarioContract.COL_PLANO02)==1);
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Plano03",c.getInt(UsuarioContract.COL_PLANO03)==1);
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Plano04",c.getInt(UsuarioContract.COL_PLANO04)==1);
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Plano05",c.getInt(UsuarioContract.COL_PLANO05)==1);
		
			
		
		json.put("DataUltimaAlteracao",c.getString(UsuarioContract.COL_DATA_ULTIMA_ALTERACAO));
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("PermiteSincronizar",c.getInt(UsuarioContract.COL_PERMITE_SINCRONIZAR)==1);
		
			
		
		json.put("CodigoExterno",c.getString(UsuarioContract.COL_CODIGO_EXTERNO));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(UsuarioContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(UsuarioContract.COLUNA_ID_USUARIO, obj.getString("IdUsuario"));
		
		valores.put(UsuarioContract.COLUNA_NOME_USUARIO, obj.getString("NomeUsuario"));
		
		valores.put(UsuarioContract.COLUNA_EMAIL, obj.getString("Email"));
			
		int valPlano01 = 0;
		if ("True".equals(obj.getString("Plano01"))) {
			valPlano01 = 1;
		}
		valores.put(UsuarioContract.COLUNA_PLANO01, valPlano01);
			
		int valPlano02 = 0;
		if ("True".equals(obj.getString("Plano02"))) {
			valPlano02 = 1;
		}
		valores.put(UsuarioContract.COLUNA_PLANO02, valPlano02);
			
		int valPlano03 = 0;
		if ("True".equals(obj.getString("Plano03"))) {
			valPlano03 = 1;
		}
		valores.put(UsuarioContract.COLUNA_PLANO03, valPlano03);
			
		int valPlano04 = 0;
		if ("True".equals(obj.getString("Plano04"))) {
			valPlano04 = 1;
		}
		valores.put(UsuarioContract.COLUNA_PLANO04, valPlano04);
			
		int valPlano05 = 0;
		if ("True".equals(obj.getString("Plano05"))) {
			valPlano05 = 1;
		}
		valores.put(UsuarioContract.COLUNA_PLANO05, valPlano05);
		
		valores.put(UsuarioContract.COLUNA_DATA_ULTIMA_ALTERACAO, obj.getLong("DataUltimaAlteracao"));
			
		int valPermiteSincronizar = 0;
		if ("True".equals(obj.getString("PermiteSincronizar"))) {
			valPermiteSincronizar = 1;
		}
		valores.put(UsuarioContract.COLUNA_PERMITE_SINCRONIZAR, valPermiteSincronizar);
		
		valores.put(UsuarioContract.COLUNA_CODIGO_EXTERNO, obj.getString("CodigoExterno"));
