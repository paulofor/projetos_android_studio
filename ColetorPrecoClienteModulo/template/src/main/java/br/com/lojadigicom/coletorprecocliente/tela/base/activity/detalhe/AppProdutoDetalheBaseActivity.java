...
public abstract class AppProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 900 * 10;
    private long idAppProduto = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idAppProduto = extra.getInt(Constantes.APP_PRODUTO_ID);
        } catch (Exception e) {
            idAppProduto = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idAppProduto==0) {
            throw new RuntimeException("Nao recebeu o IdAppProduto");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idAppProduto);
        edt.commit();
        ...
        complementaOnCreate(idAppProduto);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = AppProdutoContract.buildAppProdutoUri(idAppProduto);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(AppProdutoContract.COM_COMPLEMENTO).build();
        uri = AppProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return AppProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = AppProdutoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        AppProduto item = (AppProduto) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(AppProduto item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idAppProduto);
