...
public class InteresseProdutoMontador implements MontadorDaoI {
	...
	private InteresseProduto principal = new InteresseProdutoVo();
	
	private boolean sinc = false;
	public InteresseProdutoMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public InteresseProdutoMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		InteresseProduto item = null;
		item = (InteresseProduto) entrada;
		item.setIdInteresseProduto(getInt(c,pos++));
		item.setNota(getInt(c,pos++));
		item.setData(getTimestampDataHora(c,pos++));
		item.setValor(getFloat(c,pos++));
		item.setEspera(getLogic(c,pos++));
		item.setMonitora(getLogic(c,pos++));
		item.setVisualizacaoConcluida(getLogic(c,pos++));
		item.setPrecoMedio(getFloat(c,pos++));
		item.setPrecoMinimo(getFloat(c,pos++));
		item.setNovo(getLogic(c,pos++));
		item.setMudanca(getLogic(c,pos++));
		item.setDiferencaAnterior(getFloat(c,pos++));
		item.setMinhaAvaliacao(getInt(c,pos++));
		item.setDataUltimaMudanca(getTimestampDataHora(c,pos++));
		item.setPrecoAnterior(getFloat(c,pos++));
		item.setDataUltimaVerificacao(getTimestampDataHora(c,pos++));
		item.setIdProdutoClienteRa(getInt(c,pos++));
