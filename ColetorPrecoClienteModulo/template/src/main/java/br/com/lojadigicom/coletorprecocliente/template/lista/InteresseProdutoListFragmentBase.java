...
public abstract class InteresseProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private InteresseProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 883;
	...
	public InteresseProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, InteresseProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.INTERESSE_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(InteresseProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new InteresseProdutoListAdapterBase.InteresseProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, InteresseProdutoListViewHolderBase vh) {
    ...
    protected abstract InteresseProdutoListAdapterBase getListAdapter(FragmentActivity activity, InteresseProdutoListAdapterBase.InteresseProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(InteresseProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.INTERESSE_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.INTERESSE_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<InteresseProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista InteresseProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<InteresseProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return InteresseProdutoContract.getMontador();
    }

    protected final List<InteresseProduto> converteLista(Cursor data) {
