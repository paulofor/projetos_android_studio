...
public abstract class CompartilhamentoProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 911 * 10;
    private long idCompartilhamentoProduto = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idCompartilhamentoProduto = extra.getInt(Constantes.COMPARTILHAMENTO_PRODUTO_ID);
        } catch (Exception e) {
            idCompartilhamentoProduto = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idCompartilhamentoProduto==0) {
            throw new RuntimeException("Nao recebeu o IdCompartilhamentoProduto");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idCompartilhamentoProduto);
        edt.commit();
        ...
        complementaOnCreate(idCompartilhamentoProduto);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = CompartilhamentoProdutoContract.buildCompartilhamentoProdutoUri(idCompartilhamentoProduto);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(CompartilhamentoProdutoContract.COM_COMPLEMENTO).build();
        uri = CompartilhamentoProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return CompartilhamentoProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = CompartilhamentoProdutoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        CompartilhamentoProduto item = (CompartilhamentoProduto) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(CompartilhamentoProduto item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idCompartilhamentoProduto);
