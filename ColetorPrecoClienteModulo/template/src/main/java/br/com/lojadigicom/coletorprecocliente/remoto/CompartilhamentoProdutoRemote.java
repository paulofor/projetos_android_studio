...
public class CompartilhamentoProdutoRemote {
    ...
    private String getUrl(String metodo) {
        String objeto = "CompartilhamentoProduto";
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CompartilhamentoProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CompartilhamentoProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_compartilhamento_produto=" + valor.getAsString("id_compartilhamento_produto");
                contexto.getContentResolver().update(CompartilhamentoProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCompartilhamentoProduto",c.getString(CompartilhamentoProdutoContract.COL_ID_COMPARTILHAMENTO_PRODUTO));
		
			
		
		json.put("IdProdutoRa",c.getString(CompartilhamentoProdutoContract.COL_ID_PRODUTO_RA));
		
			
		
		json.put("DataHora",c.getString(CompartilhamentoProdutoContract.COL_DATA_HORA));
		
			
		
		json.put("IdUsuarioS",c.getString(CompartilhamentoProdutoContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CompartilhamentoProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CompartilhamentoProdutoContract.COLUNA_ID_COMPARTILHAMENTO_PRODUTO, obj.getString("IdCompartilhamentoProduto"));
		
		valores.put(CompartilhamentoProdutoContract.COLUNA_ID_PRODUTO_RA, obj.getString("IdProdutoRa"));
		
		valores.put(CompartilhamentoProdutoContract.COLUNA_DATA_HORA, obj.getLong("DataHora"));
		
		valores.put(CompartilhamentoProdutoContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
