...
public class DispositivoUsuarioMontador implements MontadorDaoI {
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item.setTokenGcmMonitor(getString(c,pos++));
		item.setMicroSd(getInt(c,pos++));
		item.setDataChamada(getTimestampDataHora(c,pos++));
		item.setTemMudanca(getLogic(c,pos++));
		item.setAtivo(getLogic(c,pos++));
		item.setIdUsuarioRa(getInt(c,pos++));
		item.setIdAppProdutoU(getInt(c,pos++));
