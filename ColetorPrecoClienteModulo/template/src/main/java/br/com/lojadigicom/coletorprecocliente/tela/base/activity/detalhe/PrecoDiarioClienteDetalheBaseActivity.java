...
public abstract class PrecoDiarioClienteDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 864 * 10;
    private long idPrecoDiarioCliente = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idPrecoDiarioCliente = extra.getInt(Constantes.PRECO_DIARIO_CLIENTE_ID);
        } catch (Exception e) {
            idPrecoDiarioCliente = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idPrecoDiarioCliente==0) {
            throw new RuntimeException("Nao recebeu o IdPrecoDiarioCliente");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idPrecoDiarioCliente);
        edt.commit();
        ...
        complementaOnCreate(idPrecoDiarioCliente);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = PrecoDiarioClienteContract.buildPrecoDiarioClienteUri(idPrecoDiarioCliente);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PrecoDiarioClienteContract.COM_COMPLEMENTO).build();
        uri = PrecoDiarioClienteContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PrecoDiarioClienteContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = PrecoDiarioClienteContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        PrecoDiarioCliente item = (PrecoDiarioCliente) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(PrecoDiarioCliente item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idPrecoDiarioCliente);
