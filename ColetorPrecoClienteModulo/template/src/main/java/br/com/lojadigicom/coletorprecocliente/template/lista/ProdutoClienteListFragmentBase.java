...
public abstract class ProdutoClienteListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ProdutoClienteListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 882;
	...
	public ProdutoClienteListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ProdutoClienteListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PRODUTO_CLIENTE_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ProdutoClienteListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ProdutoClienteListAdapterBase.ProdutoClienteListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ProdutoClienteListViewHolderBase vh) {
    ...
    protected abstract ProdutoClienteListAdapterBase getListAdapter(FragmentActivity activity, ProdutoClienteListAdapterBase.ProdutoClienteListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ProdutoClienteListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRODUTO_CLIENTE_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRODUTO_CLIENTE_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ProdutoCliente> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ProdutoCliente " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ProdutoCliente> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ProdutoClienteContract.getMontador();
    }

    protected final List<ProdutoCliente> converteLista(Cursor data) {
