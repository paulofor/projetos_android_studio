...
public abstract class ProdutoClienteProvider {
	...
	public Cursor query(UriMatcher sUriMatcher, Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		switch (sUriMatcher.match(uri)) {
	 		...
	 		case LISTANAOESCOLHIDO:
			{
				...
				DCLog.d(DCLog.TRACE_LISTA,this,"chamar queryListaNaoEscolhido()");
				//retCursor = queryListaNaoEscolhido(uri,projection,sortOrder);
				String param = uri.getQuery();
				String sql = queryListaNaoEscolhido(param) + (sortOrder!=null?" order by " + sortOrder:"");
	 		...
	 		case OBTEMPROXIMONAOESCOLHIDO:
			{
				...
				DCLog.d(DCLog.TRACE_LISTA,this,"chamar queryObtemProximoNaoEscolhido()");
				//retCursor = queryObtemProximoNaoEscolhido(uri,projection,sortOrder);
				String param = uri.getQuery();
				String sql = queryObtemProximoNaoEscolhido(param) + (sortOrder!=null?" order by " + sortOrder:"");
				DCLog.d(DCLog.TRACE_LISTA,this,"SQL: " + sql);
				retCursor = this.queryRaw(sql);
				DCLog.d(DCLog.TRACE_LISTA,this,"Cursor: " + retCursor.getCount() + " linhas");
				break;
	...
	private String sqlWhere(Uri uri) {
		...
		if (existeItem("SemPrecoDiarioClientePossui",uri.getPathSegments())) {
			sql += ProdutoClienteContract.COLUNA_CHAVE + " not in (select " + 
					PrecoDiarioClienteContract.COLUNA_ID_PRODUTO_CLIENTE_PA + " from " +
	...
	// SemChaveEstrangeira - Sem Usuario (sempre vai ser o mesmo, redundante)
		...
		if (existeItem("ComPrecoDiarioClientePossui",uri.getPathSegments())) {
			sql += "," +  PrecoDiarioClienteContract.camposOrdenados();
		}
	...
	// SemChaveEstrangeira
		...
		if (existeItem("ComPrecoDiarioClientePossui",uri.getPathSegments())) {
			sql += " " +  ProdutoClienteContract.outerJoinPrecoDiarioCliente_Possui();
		}
	...
	protected abstract String queryListaNaoEscolhido(String param);
	
	protected abstract String queryObtemProximoNaoEscolhido(String param);
