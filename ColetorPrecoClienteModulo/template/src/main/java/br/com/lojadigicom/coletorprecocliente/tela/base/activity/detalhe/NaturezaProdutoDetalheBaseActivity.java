...
public abstract class NaturezaProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 862 * 10;
    private long idNaturezaProduto = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idNaturezaProduto = extra.getInt(Constantes.NATUREZA_PRODUTO_ID);
        } catch (Exception e) {
            idNaturezaProduto = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idNaturezaProduto==0) {
            throw new RuntimeException("Nao recebeu o IdNaturezaProduto");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idNaturezaProduto);
        edt.commit();
        ...
        complementaOnCreate(idNaturezaProduto);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = NaturezaProdutoContract.buildNaturezaProdutoUri(idNaturezaProduto);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(NaturezaProdutoContract.COM_COMPLEMENTO).build();
        uri = NaturezaProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return NaturezaProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = NaturezaProdutoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        NaturezaProduto item = (NaturezaProduto) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(NaturezaProduto item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idNaturezaProduto);
