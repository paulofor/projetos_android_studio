...
public class AppProdutoRemote {
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_app_produto=" + valor.getAsString("id_app_produto");
                contexto.getContentResolver().update(AppProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("PossuiVitrine",c.getInt(AppProdutoContract.COL_POSSUI_VITRINE)==1);
		
			
		
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("Ativo",c.getInt(AppProdutoContract.COL_ATIVO)==1);
		...
		// Tipo Logico precisa ser convertido para a leitura correta no servidor.
		json.put("PossuiPalavraChave",c.getInt(AppProdutoContract.COL_POSSUI_PALAVRA_CHAVE)==1);
