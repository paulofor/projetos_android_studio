...
public abstract class OportunidadeInteresseClienteDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 910 * 10;
    private long idOportunidadeInteresseCliente = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idOportunidadeInteresseCliente = extra.getInt(Constantes.OPORTUNIDADE_INTERESSE_CLIENTE_ID);
        } catch (Exception e) {
            idOportunidadeInteresseCliente = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idOportunidadeInteresseCliente==0) {
            throw new RuntimeException("Nao recebeu o IdOportunidadeInteresseCliente");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idOportunidadeInteresseCliente);
        edt.commit();
        ...
        complementaOnCreate(idOportunidadeInteresseCliente);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = OportunidadeInteresseClienteContract.buildOportunidadeInteresseClienteUri(idOportunidadeInteresseCliente);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(OportunidadeInteresseClienteContract.COM_COMPLEMENTO).build();
        uri = OportunidadeInteresseClienteContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return OportunidadeInteresseClienteContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = OportunidadeInteresseClienteContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        OportunidadeInteresseCliente item = (OportunidadeInteresseCliente) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(OportunidadeInteresseCliente item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idOportunidadeInteresseCliente);
