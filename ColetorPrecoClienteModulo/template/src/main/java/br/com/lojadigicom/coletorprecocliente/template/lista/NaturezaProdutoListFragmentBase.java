...
public abstract class NaturezaProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{
    ...
    private static final int FORECAST_LOADER = 862;
	...
	public NaturezaProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, NaturezaProdutoListViewHolderBase vh) {
    ...
    protected abstract void montaAminacao(NaturezaProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new NaturezaProdutoListAdapterBase.NaturezaProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, NaturezaProdutoListViewHolderBase vh) {
    ...
    protected abstract NaturezaProdutoListAdapterBase getListAdapter(FragmentActivity activity, NaturezaProdutoListAdapterBase.NaturezaProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(NaturezaProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.NATUREZA_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.NATUREZA_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<NaturezaProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista NaturezaProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<NaturezaProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return NaturezaProdutoContract.getMontador();
    }

    protected final List<NaturezaProduto> converteLista(Cursor data) {
