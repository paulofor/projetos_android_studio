...
public abstract class PalavraChavePesquisaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 890 * 10;
    private long idPalavraChavePesquisa = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idPalavraChavePesquisa = extra.getInt(Constantes.PALAVRA_CHAVE_PESQUISA_ID);
        } catch (Exception e) {
            idPalavraChavePesquisa = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idPalavraChavePesquisa==0) {
            throw new RuntimeException("Nao recebeu o IdPalavraChavePesquisa");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idPalavraChavePesquisa);
        edt.commit();
        ...
        complementaOnCreate(idPalavraChavePesquisa);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = PalavraChavePesquisaContract.buildPalavraChavePesquisaUri(idPalavraChavePesquisa);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PalavraChavePesquisaContract.COM_COMPLEMENTO).build();
        uri = PalavraChavePesquisaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PalavraChavePesquisaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = PalavraChavePesquisaContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        PalavraChavePesquisa item = (PalavraChavePesquisa) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(PalavraChavePesquisa item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idPalavraChavePesquisa);
