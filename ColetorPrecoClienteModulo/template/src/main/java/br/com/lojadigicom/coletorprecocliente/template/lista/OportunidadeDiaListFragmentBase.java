...
public abstract class OportunidadeDiaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private OportunidadeDiaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 863;
	...
	public OportunidadeDiaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, OportunidadeDiaListViewHolderBase vh) {
    ...
    protected abstract void montaAminacao(OportunidadeDiaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new OportunidadeDiaListAdapterBase.OportunidadeDiaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, OportunidadeDiaListViewHolderBase vh) {
    ...
    protected abstract OportunidadeDiaListAdapterBase getListAdapter(FragmentActivity activity, OportunidadeDiaListAdapterBase.OportunidadeDiaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(OportunidadeDiaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.OPORTUNIDADE_DIA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.OPORTUNIDADE_DIA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<OportunidadeDia> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista OportunidadeDia " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<OportunidadeDia> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return OportunidadeDiaContract.getMontador();
    }

    protected final List<OportunidadeDia> converteLista(Cursor data) {
