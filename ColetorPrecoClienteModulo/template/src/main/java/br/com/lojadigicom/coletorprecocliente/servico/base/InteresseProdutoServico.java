...
public abstract class InteresseProdutoServico {



 	public final void insereAtualiza(InteresseProduto item, Context context) {
        if (item.getIdObj()==0) {
    ...
    public final void insere(InteresseProduto item, Context ctx) {
        Uri uriInsert = InteresseProdutoContract.buildInsereSinc();
        ctx.getContentResolver().insert(uriInsert, item.getContentValues());
    }
    public final void atualiza(InteresseProduto item, Context ctx) {
