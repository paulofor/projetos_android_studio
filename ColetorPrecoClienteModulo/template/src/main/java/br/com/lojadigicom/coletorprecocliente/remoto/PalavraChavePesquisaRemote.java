...
public class PalavraChavePesquisaRemote {
    ...
    private String getUrl(String metodo) {
        String objeto = "PalavraChavePesquisa";
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(PalavraChavePesquisaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(PalavraChavePesquisaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_palavra_chave_pesquisa=" + valor.getAsString("id_palavra_chave_pesquisa");
                contexto.getContentResolver().update(PalavraChavePesquisaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdPalavraChavePesquisa",c.getString(PalavraChavePesquisaContract.COL_ID_PALAVRA_CHAVE_PESQUISA));
		
			
		
		json.put("TextoBusca",c.getString(PalavraChavePesquisaContract.COL_TEXTO_BUSCA));
		
			
		
		json.put("Data",c.getString(PalavraChavePesquisaContract.COL_DATA));
		
			
		
		json.put("IdUsuarioS",c.getString(PalavraChavePesquisaContract.COL_ID_USUARIO_S));
		
			
		
		json.put("IdNaturezaProdutoRa",c.getString(PalavraChavePesquisaContract.COL_ID_NATUREZA_PRODUTO_RA));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(PalavraChavePesquisaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(PalavraChavePesquisaContract.COLUNA_ID_PALAVRA_CHAVE_PESQUISA, obj.getString("IdPalavraChavePesquisa"));
		
		valores.put(PalavraChavePesquisaContract.COLUNA_TEXTO_BUSCA, obj.getString("TextoBusca"));
		
		valores.put(PalavraChavePesquisaContract.COLUNA_DATA, obj.getLong("Data"));
		
		valores.put(PalavraChavePesquisaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
		
		valores.put(PalavraChavePesquisaContract.COLUNA_ID_NATUREZA_PRODUTO_RA, obj.getString("IdNaturezaProdutoRa"));
