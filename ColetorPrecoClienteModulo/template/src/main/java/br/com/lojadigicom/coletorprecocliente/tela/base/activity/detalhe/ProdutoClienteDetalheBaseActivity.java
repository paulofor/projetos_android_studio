...
public abstract class ProdutoClienteDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 882 * 10;
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ProdutoClienteContract.COM_COMPLEMENTO).build();
        uri = ProdutoClienteContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ProdutoClienteContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
