...
public class PrecoDiarioClienteRemote {
    ...
    private String getUrl(String metodo) {
        String objeto = "PrecoDiarioCliente";
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(PrecoDiarioClienteContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(PrecoDiarioClienteContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_preco_diario_clientte=" + valor.getAsString("id_preco_diario_clientte");
                contexto.getContentResolver().update(PrecoDiarioClienteContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdPrecoDiarioClientte",c.getString(PrecoDiarioClienteContract.COL_ID_PRECO_DIARIO_CLIENTTE));
		
			
		
		json.put("DataHora",c.getString(PrecoDiarioClienteContract.COL_DATA_HORA));
		
			
		
		json.put("PrecoVenda",c.getString(PrecoDiarioClienteContract.COL_PRECO_VENDA));
		
			
		
		json.put("IdProdutoClientePa",c.getString(PrecoDiarioClienteContract.COL_ID_PRODUTO_CLIENTE_PA));
		
			
		
		json.put("IdUsuarioS",c.getString(PrecoDiarioClienteContract.COL_ID_USUARIO_S));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(PrecoDiarioClienteContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(PrecoDiarioClienteContract.COLUNA_ID_PRECO_DIARIO_CLIENTTE, obj.getString("IdPrecoDiarioClientte"));
		
		valores.put(PrecoDiarioClienteContract.COLUNA_DATA_HORA, obj.getLong("DataHora"));
		
		valores.put(PrecoDiarioClienteContract.COLUNA_PRECO_VENDA, obj.getString("PrecoVenda"));
		
		valores.put(PrecoDiarioClienteContract.COLUNA_ID_PRODUTO_CLIENTE_PA, obj.getString("IdProdutoClientePa"));
		
		valores.put(PrecoDiarioClienteContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
