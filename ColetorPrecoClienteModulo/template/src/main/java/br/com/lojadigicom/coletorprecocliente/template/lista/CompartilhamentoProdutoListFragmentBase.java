...
public abstract class CompartilhamentoProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private CompartilhamentoProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 911;
	...
	public CompartilhamentoProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, CompartilhamentoProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.COMPARTILHAMENTO_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(CompartilhamentoProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new CompartilhamentoProdutoListAdapterBase.CompartilhamentoProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, CompartilhamentoProdutoListViewHolderBase vh) {
    ...
    protected abstract CompartilhamentoProdutoListAdapterBase getListAdapter(FragmentActivity activity, CompartilhamentoProdutoListAdapterBase.CompartilhamentoProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(CompartilhamentoProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.COMPARTILHAMENTO_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.coletorprecocliente", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.COMPARTILHAMENTO_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<CompartilhamentoProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista CompartilhamentoProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<CompartilhamentoProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return CompartilhamentoProdutoContract.getMontador();
    }

    protected final List<CompartilhamentoProduto> converteLista(Cursor data) {
