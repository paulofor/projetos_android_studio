...
public class UsuarioPesquisaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(UsuarioPesquisaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(UsuarioPesquisaContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_usuario_pesquisa=" + valor.getAsString("id_usuario_pesquisa");
                contexto.getContentResolver().update(UsuarioPesquisaContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdUsuarioPesquisa",c.getString(UsuarioPesquisaContract.COL_ID_USUARIO_PESQUISA));
		
			
		
		json.put("IdUsuarioS",c.getString(UsuarioPesquisaContract.COL_ID_USUARIO_S));
		
			
		
		json.put("IdNaturezaProdutoP",c.getString(UsuarioPesquisaContract.COL_ID_NATUREZA_PRODUTO_P));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(UsuarioPesquisaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(UsuarioPesquisaContract.COLUNA_ID_USUARIO_PESQUISA, obj.getString("IdUsuarioPesquisa"));
		
		valores.put(UsuarioPesquisaContract.COLUNA_ID_USUARIO_S, obj.getString("IdUsuarioS"));
		
		valores.put(UsuarioPesquisaContract.COLUNA_ID_NATUREZA_PRODUTO_P, obj.getString("IdNaturezaProdutoP"));
