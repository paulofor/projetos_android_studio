...
import br.com.lojadigicom.coletorprecocliente.modelo.AppProdutoVo;
...
public class AppProdutoMontador implements MontadorDaoI {
	...
	private AppProduto principal = new AppProdutoVo();
	
	private boolean sinc = false;
	public AppProdutoMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public AppProdutoMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		AppProduto item = null;
		item = (AppProduto) entrada;
		item.setIdAppProduto(getInt(c,pos++));
		item.setNome(getString(c,pos++));
		item.setUrlInstalacao(getString(c,pos++));
		item.setPossuiVitrine(getLogic(c,pos++));
		item.setAtivo(getLogic(c,pos++));
		item.setStatus(getString(c,pos++));
		item.setLimitePosicionador(getInt(c,pos++));
		item.setPossuiPalavraChave(getLogic(c,pos++));
		item.setCodigoHash(getString(c,pos++));
