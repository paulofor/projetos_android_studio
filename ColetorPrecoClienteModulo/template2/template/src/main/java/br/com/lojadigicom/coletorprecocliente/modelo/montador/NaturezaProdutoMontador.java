...
import br.com.lojadigicom.coletorprecocliente.modelo.NaturezaProdutoVo;
...
public class NaturezaProdutoMontador implements MontadorDaoI {
	...
	private NaturezaProduto principal = new NaturezaProdutoVo();
	
	private boolean sinc = false;
	public NaturezaProdutoMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public NaturezaProdutoMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		NaturezaProduto item = null;
		item = (NaturezaProduto) entrada;
		item.setIdNaturezaProduto(getInt(c,pos++));
		item.setNomeNaturezaProduto(getString(c,pos++));
		item.setIdAppProdutoAp(getInt(c,pos++));
		
		
		item.setQtdeOportunidadeDia(getInt(c,pos++)); 
