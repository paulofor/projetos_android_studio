...
public class NegocioRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(NegocioContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(NegocioContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_negocio=" + valor.getAsString("id_negocio");
                contexto.getContentResolver().update(NegocioContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdNegocio",c.getString(NegocioContract.COL_ID_NEGOCIO));
		
			
		
		json.put("Descricao",c.getString(NegocioContract.COL_DESCRICAO));
		
			
		
		json.put("DataCriacao",c.getString(NegocioContract.COL_DATA_CRIACAO));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(NegocioContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(NegocioContract.COLUNA_ID_NEGOCIO, obj.getString("IdNegocio"));
		
		valores.put(NegocioContract.COLUNA_DESCRICAO, obj.getString("Descricao"));
		
		valores.put(NegocioContract.COLUNA_DATA_CRIACAO, obj.getLong("DataCriacao"));
