...
public abstract class NegocioDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 906 * 10;
    private long idNegocio = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idNegocio = extra.getInt(Constantes.NEGOCIO_ID);
        } catch (Exception e) {
            idNegocio = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idNegocio==0) {
            throw new RuntimeException("Nao recebeu o IdNegocio");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idNegocio);
        edt.commit();
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = NegocioContract.buildNegocioUri(idNegocio);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(NegocioContract.COM_COMPLEMENTO).build();
        uri = NegocioContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return NegocioContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = NegocioContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        Negocio item = (Negocio) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(Negocio item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idNegocio);
