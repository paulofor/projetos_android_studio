...
public abstract class ItemCustoProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 894 * 10;
    private long idItemCustoProduto = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idItemCustoProduto = extra.getInt(Constantes.ITEM_CUSTO_PRODUTO_ID);
        } catch (Exception e) {
            idItemCustoProduto = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idItemCustoProduto==0) {
            throw new RuntimeException("Nao recebeu o IdItemCustoProduto");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idItemCustoProduto);
        edt.commit();
        ...
        complementaOnCreate(idItemCustoProduto);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = ItemCustoProdutoContract.buildItemCustoProdutoUri(idItemCustoProduto);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(ItemCustoProdutoContract.COM_COMPLEMENTO).build();
        uri = ItemCustoProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return ItemCustoProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = ItemCustoProdutoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        ItemCustoProduto item = (ItemCustoProduto) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(ItemCustoProduto item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idItemCustoProduto);
