...
public abstract class CenarioListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private CenarioListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 897;
	...
	public CenarioListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, CenarioListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CENARIO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(CenarioListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new CenarioListAdapterBase.CenarioListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, CenarioListViewHolderBase vh) {
    ...
    protected abstract CenarioListAdapterBase getListAdapter(FragmentActivity activity, CenarioListAdapterBase.CenarioListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(CenarioListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CENARIO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CENARIO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<Cenario> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista Cenario " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<Cenario> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return CenarioContract.getMontador();
    }

    protected final List<Cenario> converteLista(Cursor data) {
