...
import br.com.lojadigicom.capitalexterno.modelo.DispositivoUsuarioVo;
...
public class DispositivoUsuarioMontador implements MontadorDaoI {
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item.setIdDispostivoUsuario(getInt(c,pos++));
		...
		item.setMicroSd(getInt(c,pos++));
		item.setDataCriacao(getTimestampDataHora(c,pos++));
		item.setDataUltimoAcesso(getTimestampDataHora(c,pos++));
