...
public abstract class PrevisaoVendaDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 898 * 10;
    private long idPrevisaoVenda = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idPrevisaoVenda = extra.getInt(Constantes.PREVISAO_VENDA_ID);
        } catch (Exception e) {
            idPrevisaoVenda = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idPrevisaoVenda==0) {
            throw new RuntimeException("Nao recebeu o IdPrevisaoVenda");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idPrevisaoVenda);
        edt.commit();
        ...
        complementaOnCreate(idPrevisaoVenda);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = PrevisaoVendaContract.buildPrevisaoVendaUri(idPrevisaoVenda);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PrevisaoVendaContract.COM_COMPLEMENTO).build();
        uri = PrevisaoVendaContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PrevisaoVendaContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = PrevisaoVendaContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        PrevisaoVenda item = (PrevisaoVenda) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(PrevisaoVenda item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idPrevisaoVenda);
