...
public class ItemCustoProdutoRemote {
    ...
    private String getUrl(String metodo) {
        String objeto = "ItemCustoProduto";
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(ItemCustoProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(ItemCustoProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_item_custo_produto=" + valor.getAsString("id_item_custo_produto");
                contexto.getContentResolver().update(ItemCustoProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdItemCustoProduto",c.getString(ItemCustoProdutoContract.COL_ID_ITEM_CUSTO_PRODUTO));
		
			
		
		json.put("Valor",c.getString(ItemCustoProdutoContract.COL_VALOR));
		
			
		
		json.put("Descricao",c.getString(ItemCustoProdutoContract.COL_DESCRICAO));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(ItemCustoProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(ItemCustoProdutoContract.COLUNA_ID_ITEM_CUSTO_PRODUTO, obj.getString("IdItemCustoProduto"));
		
		valores.put(ItemCustoProdutoContract.COLUNA_VALOR, obj.getString("Valor"));
		
		valores.put(ItemCustoProdutoContract.COLUNA_DESCRICAO, obj.getString("Descricao"));
