...
public abstract class NegocioListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private NegocioListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 906;
	...
	public NegocioListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, NegocioListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.NEGOCIO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(NegocioListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new NegocioListAdapterBase.NegocioListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, NegocioListViewHolderBase vh) {
    ...
    protected abstract NegocioListAdapterBase getListAdapter(FragmentActivity activity, NegocioListAdapterBase.NegocioListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(NegocioListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.NEGOCIO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.NEGOCIO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<Negocio> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista Negocio " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<Negocio> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return NegocioContract.getMontador();
    }

    protected final List<Negocio> converteLista(Cursor data) {
