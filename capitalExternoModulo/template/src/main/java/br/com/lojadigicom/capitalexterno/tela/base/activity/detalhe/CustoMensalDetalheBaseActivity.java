...
public abstract class CustoMensalDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 895 * 10;
    private long idCustoMensal = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idCustoMensal = extra.getInt(Constantes.CUSTO_MENSAL_ID);
        } catch (Exception e) {
            idCustoMensal = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idCustoMensal==0) {
            throw new RuntimeException("Nao recebeu o IdCustoMensal");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idCustoMensal);
        edt.commit();
        ...
        complementaOnCreate(idCustoMensal);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = CustoMensalContract.buildCustoMensalUri(idCustoMensal);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(CustoMensalContract.COM_COMPLEMENTO).build();
        uri = CustoMensalContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return CustoMensalContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = CustoMensalContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        CustoMensal item = (CustoMensal) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(CustoMensal item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idCustoMensal);
