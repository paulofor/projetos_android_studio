...
import br.com.lojadigicom.capitalexterno.modelo.CenarioVo;
...
public class CenarioMontador implements MontadorDaoI {

	private Cenario principal = new CenarioVo();
	
	private boolean sinc = false;
	public CenarioMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public CenarioMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c) {
		...
		//objeto = FabricaVo.criaCenario();
		objeto = new CenarioVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		Cenario item = null;
		item = (Cenario) entrada;
		item.setIdCenario(getInt(c,pos++));
		item.setDescricaoCenario(getString(c,pos++));
