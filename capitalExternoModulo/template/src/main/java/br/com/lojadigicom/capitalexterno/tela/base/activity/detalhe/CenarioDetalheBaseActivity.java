...
public abstract class CenarioDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 897 * 10;
    private long idCenario = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idCenario = extra.getInt(Constantes.CENARIO_ID);
        } catch (Exception e) {
            idCenario = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idCenario==0) {
            throw new RuntimeException("Nao recebeu o IdCenario");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idCenario);
        edt.commit();
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = CenarioContract.buildCenarioUri(idCenario);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(CenarioContract.COM_COMPLEMENTO).build();
        uri = CenarioContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return CenarioContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = CenarioContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        Cenario item = (Cenario) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(Cenario item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idCenario);
