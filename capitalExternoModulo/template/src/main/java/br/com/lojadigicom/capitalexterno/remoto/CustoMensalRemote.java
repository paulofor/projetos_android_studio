...
public class CustoMensalRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CustoMensalContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CustoMensalContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_custo_mensal=" + valor.getAsString("id_custo_mensal");
                contexto.getContentResolver().update(CustoMensalContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCustoMensal",c.getString(CustoMensalContract.COL_ID_CUSTO_MENSAL));
		
			
		
		json.put("Descricao",c.getString(CustoMensalContract.COL_DESCRICAO));
		
			
		
		json.put("ValorMedio",c.getString(CustoMensalContract.COL_VALOR_MEDIO));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CustoMensalContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CustoMensalContract.COLUNA_ID_CUSTO_MENSAL, obj.getString("IdCustoMensal"));
		
		valores.put(CustoMensalContract.COLUNA_DESCRICAO, obj.getString("Descricao"));
		
		valores.put(CustoMensalContract.COLUNA_VALOR_MEDIO, obj.getString("ValorMedio"));
