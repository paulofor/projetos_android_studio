...
public abstract class MesAnoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private MesAnoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 899;
	...
	public MesAnoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, MesAnoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.MES_ANO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(MesAnoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new MesAnoListAdapterBase.MesAnoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, MesAnoListViewHolderBase vh) {
    ...
    protected abstract MesAnoListAdapterBase getListAdapter(FragmentActivity activity, MesAnoListAdapterBase.MesAnoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(MesAnoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.MES_ANO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.MES_ANO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<MesAno> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista MesAno " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<MesAno> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return MesAnoContract.getMontador();
    }

    protected final List<MesAno> converteLista(Cursor data) {
