...
public abstract class LinhaProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private LinhaProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 892;
	...
	public LinhaProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, LinhaProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.LINHA_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(LinhaProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new LinhaProdutoListAdapterBase.LinhaProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, LinhaProdutoListViewHolderBase vh) {
    ...
    protected abstract LinhaProdutoListAdapterBase getListAdapter(FragmentActivity activity, LinhaProdutoListAdapterBase.LinhaProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(LinhaProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.LINHA_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.LINHA_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<LinhaProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista LinhaProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<LinhaProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return LinhaProdutoContract.getMontador();
    }

    protected final List<LinhaProduto> converteLista(Cursor data) {
