...
public abstract class ItemCustoProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ItemCustoProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 894;
	...
	public ItemCustoProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ItemCustoProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.ITEM_CUSTO_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ItemCustoProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ItemCustoProdutoListAdapterBase.ItemCustoProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ItemCustoProdutoListViewHolderBase vh) {
    ...
    protected abstract ItemCustoProdutoListAdapterBase getListAdapter(FragmentActivity activity, ItemCustoProdutoListAdapterBase.ItemCustoProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ItemCustoProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.ITEM_CUSTO_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.ITEM_CUSTO_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ItemCustoProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ItemCustoProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<ItemCustoProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ItemCustoProdutoContract.getMontador();
    }

    protected final List<ItemCustoProduto> converteLista(Cursor data) {
