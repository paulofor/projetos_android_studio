...
public abstract class CustoMensalListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private CustoMensalListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 895;
	...
	public CustoMensalListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, CustoMensalListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.CUSTO_MENSAL_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(CustoMensalListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new CustoMensalListAdapterBase.CustoMensalListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, CustoMensalListViewHolderBase vh) {
    ...
    protected abstract CustoMensalListAdapterBase getListAdapter(FragmentActivity activity, CustoMensalListAdapterBase.CustoMensalListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(CustoMensalListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.CUSTO_MENSAL_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.CUSTO_MENSAL_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<CustoMensal> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista CustoMensal " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<CustoMensal> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return CustoMensalContract.getMontador();
    }

    protected final List<CustoMensal> converteLista(Cursor data) {
