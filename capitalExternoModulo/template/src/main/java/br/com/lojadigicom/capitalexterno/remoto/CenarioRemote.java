...
public class CenarioRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(CenarioContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(CenarioContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_cenario=" + valor.getAsString("id_cenario");
                contexto.getContentResolver().update(CenarioContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdCenario",c.getString(CenarioContract.COL_ID_CENARIO));
		
			
		
		json.put("DescricaoCenario",c.getString(CenarioContract.COL_DESCRICAO_CENARIO));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(CenarioContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(CenarioContract.COLUNA_ID_CENARIO, obj.getString("IdCenario"));
		
		valores.put(CenarioContract.COLUNA_DESCRICAO_CENARIO, obj.getString("DescricaoCenario"));
