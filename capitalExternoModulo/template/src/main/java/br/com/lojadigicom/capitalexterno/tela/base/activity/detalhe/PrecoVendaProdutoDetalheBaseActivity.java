...
public abstract class PrecoVendaProdutoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 896 * 10;
    private long idPrecoVendaProduto = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idPrecoVendaProduto = extra.getInt(Constantes.PRECO_VENDA_PRODUTO_ID);
        } catch (Exception e) {
            idPrecoVendaProduto = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idPrecoVendaProduto==0) {
            throw new RuntimeException("Nao recebeu o IdPrecoVendaProduto");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idPrecoVendaProduto);
        edt.commit();
        ...
        complementaOnCreate(idPrecoVendaProduto);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = PrecoVendaProdutoContract.buildPrecoVendaProdutoUri(idPrecoVendaProduto);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(PrecoVendaProdutoContract.COM_COMPLEMENTO).build();
        uri = PrecoVendaProdutoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return PrecoVendaProdutoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = PrecoVendaProdutoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        PrecoVendaProduto item = (PrecoVendaProduto) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(PrecoVendaProduto item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idPrecoVendaProduto);
