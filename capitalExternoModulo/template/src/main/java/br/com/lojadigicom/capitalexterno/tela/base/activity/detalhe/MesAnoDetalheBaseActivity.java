...
public abstract class MesAnoDetalheBaseActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, Transition.TransitionListener {

	String titulo = null;
    private static final int DETAIL_LOADER = 899 * 10;
    private long idMesAno = 0;
	...
	protected final void onCreate(Bundle savedInstanceState) {
	    ...
	    SharedPreferences pref = getPreferences(0);
 		...
 		titulo = (extra!=null?extra.getString(Constantes.CHAVE_TOOLBAR):null);
   		try {
            this.idMesAno = extra.getInt(Constantes.MES_ANO_ID);
        } catch (Exception e) {
            idMesAno = pref.getLong(this.getClass().getSimpleName(), 0);
        }
  
        if (idMesAno==0) {
            throw new RuntimeException("Nao recebeu o IdMesAno");
        }
        SharedPreferences.Editor edt = pref.edit();
        edt.putLong(this.getClass().getSimpleName(), idMesAno);
        edt.commit();
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        Uri uri = MesAnoContract.buildMesAnoUri(idMesAno);
        uri = completaUri(uri);
	...
	@Override
    protected Uri completaUri(Uri uri) {
        uri = uri.buildUpon().appendPath(MesAnoContract.COM_COMPLEMENTO).build();
        uri = MesAnoContract.adiciona<Objeto><Relacionamento>(uri);
        return uri;
    } 
	*/
	...
	@Override
    protected MontadorDaoI completaMontador(MontadorDaoComposite montadorDao) {
        return MesAnoContract.adicionaMontador<Objeto><Relacionamento>(montadorDao);
    }
	*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        MontadorDaoI montadorDao = MesAnoContract.getMontadorComposto();
        montadorDao = completaMontador((MontadorDaoComposite)montadorDao);
        MesAno item = (MesAno) montadorDao.getItem(data);
        onLoadObject(item);
    }
   
    
	protected abstract void onLoadObject(MesAno item);

	protected abstract int getConstColunaNomeTitulo();

    protected abstract void complementaOnCreate(long idMesAno);
