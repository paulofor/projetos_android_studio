...
public abstract class PrecoVendaProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PrecoVendaProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 896;
	...
	public PrecoVendaProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, PrecoVendaProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PRECO_VENDA_PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(PrecoVendaProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new PrecoVendaProdutoListAdapterBase.PrecoVendaProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PrecoVendaProdutoListViewHolderBase vh) {
    ...
    protected abstract PrecoVendaProdutoListAdapterBase getListAdapter(FragmentActivity activity, PrecoVendaProdutoListAdapterBase.PrecoVendaProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PrecoVendaProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRECO_VENDA_PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRECO_VENDA_PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PrecoVendaProduto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PrecoVendaProduto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<PrecoVendaProduto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PrecoVendaProdutoContract.getMontador();
    }

    protected final List<PrecoVendaProduto> converteLista(Cursor data) {
