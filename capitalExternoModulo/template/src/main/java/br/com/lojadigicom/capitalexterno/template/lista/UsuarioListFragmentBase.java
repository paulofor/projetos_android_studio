...
public abstract class UsuarioListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private UsuarioListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 907;
	...
	public UsuarioListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, UsuarioListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.USUARIO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(UsuarioListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new UsuarioListAdapterBase.UsuarioListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, UsuarioListViewHolderBase vh) {
    ...
    protected abstract UsuarioListAdapterBase getListAdapter(FragmentActivity activity, UsuarioListAdapterBase.UsuarioListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(UsuarioListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.USUARIO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.USUARIO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<Usuario> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista Usuario " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<Usuario> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return UsuarioContract.getMontador();
    }

    protected final List<Usuario> converteLista(Cursor data) {
