...
public abstract class ProdutoListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ProdutoListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 893;
	...
	public ProdutoListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, ProdutoListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PRODUTO_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(ProdutoListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ProdutoListAdapterBase.ProdutoListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ProdutoListViewHolderBase vh) {
    ...
    protected abstract ProdutoListAdapterBase getListAdapter(FragmentActivity activity, ProdutoListAdapterBase.ProdutoListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ProdutoListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRODUTO_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRODUTO_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<Produto> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista Produto " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<Produto> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ProdutoContract.getMontador();
    }

    protected final List<Produto> converteLista(Cursor data) {
	...
	protected long mIdLinhaProduto;
	public final void setIdLinhaProduto(long id) {
		mIdLinhaProduto = id;
	}
