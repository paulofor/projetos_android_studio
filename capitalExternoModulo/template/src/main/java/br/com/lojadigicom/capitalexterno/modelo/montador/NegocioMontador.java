...
import br.com.lojadigicom.capitalexterno.modelo.NegocioVo;
...
public class NegocioMontador implements MontadorDaoI {

	private Negocio principal = new NegocioVo();
	
	private boolean sinc = false;
	public NegocioMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public NegocioMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c) {
		...
		//objeto = FabricaVo.criaNegocio();
		objeto = new NegocioVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		Negocio item = null;
		item = (Negocio) entrada;
		item.setIdNegocio(getInt(c,pos++));
		item.setDescricao(getString(c,pos++));
		item.setDataCriacao(getTimestampDataHora(c,pos++));
