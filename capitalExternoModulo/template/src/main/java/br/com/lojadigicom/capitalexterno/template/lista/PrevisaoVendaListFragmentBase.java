...
public abstract class PrevisaoVendaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private PrevisaoVendaListAdapterBase mAdapter;

    private int mPosition = RecyclerView.NO_POSITION;
    private int mChoiceMode;
    private static final String SELECTED_KEY = "selected_position";
    private static final int FORECAST_LOADER = 898;
	...
	public PrevisaoVendaListFragmentBase() {
    }


	private final void onItemSelected(int idItemLista, PrevisaoVendaListViewHolderBase vh) {
        ...
        intent.putExtra(Constantes.PREVISAO_VENDA_ID,idItemLista);
        if (idItemLista==0) {
    ...
    protected abstract void montaAminacao(PrevisaoVendaListViewHolderBase vh, ArrayList<Pair<View,String>> lista);
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new PrevisaoVendaListAdapterBase.PrevisaoVendaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, PrevisaoVendaListViewHolderBase vh) {
    ...
    protected abstract PrevisaoVendaListAdapterBase getListAdapter(FragmentActivity activity, PrevisaoVendaListAdapterBase.PrevisaoVendaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(PrevisaoVendaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PREVISAO_VENDA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.capitalexterno", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PREVISAO_VENDA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<PrevisaoVenda> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista PrevisaoVenda " + lista.size() + " itens");
    	DCLog.d(DCLog.TRACE_LISTA,this,"List<PrevisaoVenda> : " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return PrevisaoVendaContract.getMontador();
    }

    protected final List<PrevisaoVenda> converteLista(Cursor data) {
