...
public class PrecoVendaProdutoRemote {
    ...
    private String getUrl(String metodo) {
        String objeto = "PrecoVendaProduto";
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(PrecoVendaProdutoContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(PrecoVendaProdutoContract.getContentUri(), vetor);
  	...
  	public int listaSincronizadaDaoAtualizaPorId(Context contexto, boolean delete, String codigoAplicacao) throws JSONException {
        ...
        try {
            ...
            for (int x = 0 ; x < lista.length() ; x++) {
                ...
                ContentValues valor = this.ContentValuesAtributos(item);
                //vetor[x] = valor;
                String where = "id_preco_venda_produto=" + valor.getAsString("id_preco_venda_produto");
                contexto.getContentResolver().update(PrecoVendaProdutoContract.buildAtualiza(),valor,where,null);
            }
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
		...
		json.put("IdPrecoVendaProduto",c.getString(PrecoVendaProdutoContract.COL_ID_PRECO_VENDA_PRODUTO));
		
			
		
		json.put("Valor",c.getString(PrecoVendaProdutoContract.COL_VALOR));
     ...
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        ...
        json.put("OperacaoSinc",c.getString(PrecoVendaProdutoContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(PrecoVendaProdutoContract.COLUNA_ID_PRECO_VENDA_PRODUTO, obj.getString("IdPrecoVendaProduto"));
		
		valores.put(PrecoVendaProdutoContract.COLUNA_VALOR, obj.getString("Valor"));
