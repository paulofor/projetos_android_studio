...
public abstract class ProdutoPesquisaListFragmentBase extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>{

    private RecyclerView mRecyclerView;
    private ProdutoPesquisaListAdapterBase mAdapter;
	...
	public abstract void onItemSelected(int idItemLista, ProdutoPesquisaListViewHolderBase vh);
	

    public ProdutoPesquisaListFragmentBase() {
    }
    ...
    public final View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = getListAdapter(this.getActivity(), new ProdutoPesquisaListAdapterBase.ProdutoPesquisaListAdapterOnClickHandler() {
            @Override
            public void onClick(int idItemLista, ProdutoPesquisaListViewHolderBase vh) {
    ...
    protected abstract ProdutoPesquisaListAdapterBase getListAdapter(FragmentActivity activity, ProdutoPesquisaListAdapterBase.ProdutoPesquisaListAdapterOnClickHandler onClickHandler, View emptyView, int mChoiceMode);

	/*
 	public void registraPosicao(ProdutoPesquisaListViewHolderBase vh) {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        SharedPreferences.Editor edt = pref.edit();
        edt.putInt(Constantes.PRODUTO_PESQUISA_POSICAO, vh.getAdapterPosition());
        edt.commit();
    }
    private void recuperaPosicao() {
        SharedPreferences pref = getActivity().getSharedPreferences("br.com.lojadigicom.precomed", Context.MODE_PRIVATE);
        mPosition = pref.getInt(Constantes.PRODUTO_PESQUISA_POSICAO,RecyclerView.NO_POSITION);
    }
	*/
    ...
    public final void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    	List<ProdutoPesquisa> lista = converteLista(data);
    	DCLog.d(DCLog.OPERACAO_DB_TELA,this,"Lista ProdutoPesquisa " + lista.size() + " itens");
	...
	protected MontadorDaoI getMontador() {
        return ProdutoPesquisaContract.getMontador();
    }

    protected final List<ProdutoPesquisa> converteLista(Cursor data) {
