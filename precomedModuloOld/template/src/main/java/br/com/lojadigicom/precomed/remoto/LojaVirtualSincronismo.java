...
public class LojaVirtualSincronismo {
	...
	public void sincroniza(Context contexto, boolean forcaAtualizacao, boolean delete, boolean atualizaLocal) {
	 	LojaVirtualRemote servicoRemoto = new  LojaVirtualRemote(); // fazer estatico ?
        try {
             ContentResolver resolver = contexto.getContentResolver();
             Cursor cursor = resolver.query(LojaVirtualContract.buildAllSinc(), LojaVirtualContract.COLS_SINC, null, null, null);
             int tam = cursor.getCount();
             if (cursor.getCount() > 0 ) {
                  servicoRemoto.listaSincronizadaAlteracao(cursor, contexto);
                  DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "LojaVirtual: " + tam +  " >> ");
                  resolver.delete(LojaVirtualContract.buildDeleteAllSinc(),null,null);
             }
