...
public class MarcaRemote {
    ...
    public int listaSincronizadaDao(Context contexto, boolean delete) throws JSONException {
        ...
        try {
            ...
            if (delete) {
                contexto.getContentResolver().delete(MarcaContract.buildDeleteAllRecreate(), null, null);
            }
            contexto.getContentResolver().bulkInsert(MarcaContract.getContentUri(), vetor);
    ...
    private JSONObject JSonAtributos(Cursor c) throws JSONException {
        ...
        JSONObject json = new JSONObject();
        	json.put("IdMarca",c.getString(MarcaContract.COL_ID_MARCA));
			json.put("NomeMarca",c.getString(MarcaContract.COL_NOME_MARCA));
		
        return json;
    }
     private JSONObject JSonAtributosSinc(Cursor c) throws JSONException {
        JSONObject json = JSonAtributos(c);
        json.put("OperacaoSinc",c.getString(MarcaContract.COL_OPERACAO_SINC));
        return json;
    }
    ...
    private ContentValues ContentValuesAtributos(JSONObject obj) throws JSONException {
		...
		valores.put(MarcaContract.COLUNA_ID_MARCA, obj.getString("IdMarca"));
		
		valores.put(MarcaContract.COLUNA_NOME_MARCA, obj.getString("NomeMarca"));
