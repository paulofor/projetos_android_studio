...
public class ProdutoPesquisaSincronismo {
	...
	public void sincroniza(Context contexto, boolean forcaAtualizacao, boolean delete, boolean atualizaLocal) {
	 	ProdutoPesquisaRemote servicoRemoto = new  ProdutoPesquisaRemote(); // fazer estatico ?
        try {
             ContentResolver resolver = contexto.getContentResolver();
             Cursor cursor = resolver.query(ProdutoPesquisaContract.buildAllSinc(), ProdutoPesquisaContract.COLS_SINC, null, null, null);
             int tam = cursor.getCount();
             if (cursor.getCount() > 0 ) {
                  servicoRemoto.listaSincronizadaAlteracao(cursor, contexto);
                  DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "ProdutoPesquisa: " + tam +  " >> ");
                  resolver.delete(ProdutoPesquisaContract.buildDeleteAllSinc(),null,null);
             }
             if ((atualizaLocal) && (forcaAtualizacao || (tam>0))) {
                 int tamLista = servicoRemoto.listaSincronizadaDao(contexto,delete);
                 DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "ProdutoPesquisa: " + tamLista +  " << ");
             }
        } catch (Exception e) {
