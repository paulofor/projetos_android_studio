...
public class LojaVirtualMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (LojaVirtual) entrada;item.setIdLojaVirtual(getInt(c,pos++));
		item.setNomeLojaVirtual(getString(c,pos++));
		item.setUrlInicialBrinquedo(getString(c,pos++));
		if (sinc) {
