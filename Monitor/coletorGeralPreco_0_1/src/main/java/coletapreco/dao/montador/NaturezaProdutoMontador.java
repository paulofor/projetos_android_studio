...
public class NaturezaProdutoMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (NaturezaProduto) entrada;item.setIdNaturezaProduto(getInt(c,pos++));
		item.setNomeNaturezaProduto(getString(c,pos++));
		item.setCodigoNaturezaProduto(getString(c,pos++));
		if (sinc) {
