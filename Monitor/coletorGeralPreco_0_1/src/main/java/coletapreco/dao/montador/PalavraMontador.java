...
public class PalavraMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public PalavraMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public PalavraMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		Palavra item = null;
		item = (Palavra) entrada;item.setIdPalavra(getInt(c,pos++));
		item.setDescricao(getString(c,pos++));
		item.setComum(getLogic(c,pos++));
		if (sinc) {
