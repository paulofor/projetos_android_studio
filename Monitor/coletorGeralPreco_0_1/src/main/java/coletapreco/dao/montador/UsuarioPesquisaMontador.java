...
public class UsuarioPesquisaMontador extends MontadorDaoBase implements MontadorDaoI{
	...
	public UsuarioPesquisaMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public UsuarioPesquisaMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (UsuarioPesquisa) entrada;item.setIdUsuarioPesquisa(getInt(c,pos++));
		item.setIdUsuarioS(getInt(c,pos++));
		item.setIdNaturezaProdutoP(getInt(c,pos++));
		if (sinc) {
