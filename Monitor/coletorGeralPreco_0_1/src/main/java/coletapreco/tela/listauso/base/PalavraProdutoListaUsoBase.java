...
import coletapreco.view.adapter.listauso.PalavraProdutoListUsoAdapter;
...
public abstract class PalavraProdutoListaUsoBase extends BaseListFragment implements IQuadroLista {

	public final static String ITEM_CLICK = "PalavraProdutoItemClick";

	private PalavraProdutoListUsoAdapterBase adapt = null;
	...
	protected void atualizaLista(List<PalavraProduto> listaNova) {
	...
	*/
	...
	private void preencheLista() {
        ...
        List<PalavraProduto> lista = getListaCorrente(dContexto.getContext(),servico);
        // Pode ser necessario um adapter customizado (diferenciar o editar e usar)
        adapt =  getAdapter(lista, dContexto);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "getListaCorrente : List<PalavraProduto> -> " + lista.size() + " itens (Adapter:" + adapt.getClass().getName() + ")");
        adapt.setRaiz(this);
        setListAdapter(adapt);
	}
	
	protected PalavraProdutoListUsoAdapterBase getAdapter(List<PalavraProduto> lista,DigicomContexto dContexto) {
		return new PalavraProdutoListUsoAdapter(lista,dContexto);
	}
	
	protected List<PalavraProduto> getListaCorrente(Context contexto,PalavraProdutoServico servico) {
		
		throw new UnsupportedOperationException("Fazer override de getListaCorrente em PalavraProdutoListaUso, trata-se de classe de relacionamento");
