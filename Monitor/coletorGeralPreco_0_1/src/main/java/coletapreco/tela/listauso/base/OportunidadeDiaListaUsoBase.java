...
import coletapreco.view.adapter.listauso.OportunidadeDiaListUsoAdapter;
...
public abstract class OportunidadeDiaListaUsoBase extends BaseListFragment implements IQuadroLista {

	public final static String ITEM_CLICK = "OportunidadeDiaItemClick";

	private OportunidadeDiaListUsoAdapterBase adapt = null;
	...
	protected void atualizaLista(List<OportunidadeDia> listaNova) {
	...
	*/
	...
	private void preencheLista() {
        ...
        List<OportunidadeDia> lista = getListaCorrente(dContexto.getContext(),servico);
        // Pode ser necessario um adapter customizado (diferenciar o editar e usar)
        adapt =  getAdapter(lista, dContexto);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "getListaCorrente : List<OportunidadeDia> -> " + lista.size() + " itens (Adapter:" + adapt.getClass().getName() + ")");
        adapt.setRaiz(this);
        setListAdapter(adapt);
	}
	
	protected OportunidadeDiaListUsoAdapterBase getAdapter(List<OportunidadeDia> lista,DigicomContexto dContexto) {
		return new OportunidadeDiaListUsoAdapter(lista,dContexto);
	}
