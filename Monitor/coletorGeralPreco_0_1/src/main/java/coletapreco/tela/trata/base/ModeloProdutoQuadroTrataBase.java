...
public abstract class ModeloProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected ModeloProdutoAgregadoI getItem() {
		return (ModeloProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (ModeloProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataModeloProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para ModeloProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em ModeloProdutoQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de ModeloProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(ModeloProduto valor) {
		ModeloProdutoServico exibicaoSrv = FabricaServico.getInstancia().getModeloProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_modelo_produto, null);
	...
	// outro quadro
	protected final void insereItem(ModeloProduto valor) {
		ModeloProdutoServico exibicaoSrv = FabricaServico.getInstancia().getModeloProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_modelo_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(ModeloProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em ModeloProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final ModeloProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em ModeloProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_modelo_produto,"R.layout.trata_modelo_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaModeloProduto();
