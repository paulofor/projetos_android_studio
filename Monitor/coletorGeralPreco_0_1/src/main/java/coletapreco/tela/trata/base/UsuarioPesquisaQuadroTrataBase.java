...
public abstract class UsuarioPesquisaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected UsuarioPesquisaAgregadoI getItem() {
		return (UsuarioPesquisaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (UsuarioPesquisa) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataUsuarioPesquisa);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para UsuarioPesquisaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em UsuarioPesquisaQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de UsuarioPesquisaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(UsuarioPesquisa valor) {
		UsuarioPesquisaServico exibicaoSrv = FabricaServico.getInstancia().getUsuarioPesquisaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_usuario_pesquisa, null);
	...
	// outro quadro
	protected final void insereItem(UsuarioPesquisa valor) {
		UsuarioPesquisaServico exibicaoSrv = FabricaServico.getInstancia().getUsuarioPesquisaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_usuario_pesquisa, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(UsuarioPesquisa vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em UsuarioPesquisaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final UsuarioPesquisa vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em UsuarioPesquisaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_usuario_pesquisa,"R.layout.trata_usuario_pesquisa");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaUsuarioPesquisa();
