...
public abstract class LojaNaturezaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {

	
	private LojaNatureza item = null;
	private boolean insercao = false;
	private boolean alteracao = false;
	//protected Activity activity = null;
	
	// Para manipular apenas os dados relacionados
	protected LojaNaturezaAgregadoI getItem() {
		return (LojaNaturezaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (LojaNatureza) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataLojaNatureza);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para LojaNaturezaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em LojaNaturezaQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de LojaNaturezaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(LojaNatureza valor) {
		LojaNaturezaServico exibicaoSrv = FabricaServico.getInstancia().getLojaNaturezaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_loja_natureza, null);
	...
	// outro quadro
	protected final void insereItem(LojaNatureza valor) {
		LojaNaturezaServico exibicaoSrv = FabricaServico.getInstancia().getLojaNaturezaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_loja_natureza, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(LojaNatureza vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em LojaNaturezaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final LojaNatureza vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em LojaNaturezaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_loja_natureza,"R.layout.trata_loja_natureza");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaLojaNatureza();
