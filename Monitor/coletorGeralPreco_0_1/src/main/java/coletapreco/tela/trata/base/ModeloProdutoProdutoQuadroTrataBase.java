...
public abstract class ModeloProdutoProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected ModeloProdutoProdutoAgregadoI getItem() {
		return (ModeloProdutoProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (ModeloProdutoProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataModeloProdutoProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para ModeloProdutoProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em ModeloProdutoProdutoQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de ModeloProdutoProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(ModeloProdutoProduto valor) {
		ModeloProdutoProdutoServico exibicaoSrv = FabricaServico.getInstancia().getModeloProdutoProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_modelo_produto_produto, null);
	...
	// outro quadro
	protected final void insereItem(ModeloProdutoProduto valor) {
		ModeloProdutoProdutoServico exibicaoSrv = FabricaServico.getInstancia().getModeloProdutoProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_modelo_produto_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(ModeloProdutoProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em ModeloProdutoProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final ModeloProdutoProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em ModeloProdutoProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_modelo_produto_produto,"R.layout.trata_modelo_produto_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaModeloProdutoProduto();
