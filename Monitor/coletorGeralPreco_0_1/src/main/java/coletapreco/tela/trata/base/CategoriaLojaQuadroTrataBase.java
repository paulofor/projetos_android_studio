...
import coletapreco.modelo.agregado.CategoriaLojaAgregadoI;
...
public abstract class CategoriaLojaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {

	
	private CategoriaLoja item = null;
	private boolean insercao = false;
	private boolean alteracao = false;
	//protected Activity activity = null;
	
	// Para manipular apenas os dados relacionados
	protected CategoriaLojaAgregadoI getItem() {
		return (CategoriaLojaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (CategoriaLoja) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataCategoriaLoja);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para CategoriaLojaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em CategoriaLojaQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de CategoriaLojaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(CategoriaLoja valor) {
		CategoriaLojaServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaLojaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_loja, null);
	...
	// outro quadro
	protected final void insereItem(CategoriaLoja valor) {
		CategoriaLojaServico exibicaoSrv = FabricaServico.getInstancia().getCategoriaLojaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_categoria_loja, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(CategoriaLoja vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em CategoriaLojaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final CategoriaLoja vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em CategoriaLojaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_categoria_loja,"R.layout.trata_categoria_loja");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			CategoriaLojaServico servico = FabricaServico.getInstancia().getCategoriaLojaServico(FabricaServico.TIPO_SQLITE);
			item = FabricaVo.criaCategoriaLoja();
