...
public abstract class MarcaQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected MarcaAgregadoI getItem() {
		return (MarcaAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (Marca) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataMarca);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para MarcaQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em MarcaQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de MarcaQuadroTrataBase");
	//}
	...
	protected final void alteraItem(Marca valor) {
		MarcaServico exibicaoSrv = FabricaServico.getInstancia().getMarcaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_marca, null);
	...
	// outro quadro
	protected final void insereItem(Marca valor) {
		MarcaServico exibicaoSrv = FabricaServico.getInstancia().getMarcaServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_marca, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(Marca vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em MarcaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final Marca vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em MarcaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_marca,"R.layout.trata_marca");
		return recurso;
	}
