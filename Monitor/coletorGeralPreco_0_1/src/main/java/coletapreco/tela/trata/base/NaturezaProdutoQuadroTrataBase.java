...
public abstract class NaturezaProdutoQuadroTrataBase extends BaseDialogFragment implements IFragmentEdicao {
	...
	protected NaturezaProdutoAgregadoI getItem() {
		return (NaturezaProdutoAgregadoI) item;
	}

	
	
	public void setItem(DCIObjetoDominio item) {
		this.item = (NaturezaProduto) item;
	...
	protected void setOnOk(View tela) {
		Button btn = (Button) tela.findViewById(R.id.btnOkTrataNaturezaProduto);
		btn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!alteracao && !insercao) {
					throw new UnsupportedOperationException("Nao foi selecionado nem alteracao nem insercao para NaturezaProdutoQuadroTrata");
				} else {
					if (item==null) {
						throw new UnsupportedOperationException("Item com valor null em NaturezaProdutoQuadroTrataBase");
					}
	...
	//	throw new RuntimeException("Implementar validaCamposTela de NaturezaProdutoQuadroTrataBase");
	//}
	...
	protected final void alteraItem(NaturezaProduto valor) {
		NaturezaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getNaturezaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_natureza_produto, null);
	...
	// outro quadro
	protected final void insereItem(NaturezaProduto valor) {
		NaturezaProdutoServico exibicaoSrv = FabricaServico.getInstancia().getNaturezaProdutoServico(FabricaServico.TIPO_SQLITE);
		//View saida = LayoutInflater.from(getActivity()).inflate(R.layout.trata_natureza_produto, null);
		transfereTelaParaVo(valor);
	...
	protected void transfereTelaParaVo(NaturezaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereTelaParaVo em NaturezaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	protected void transfereVoParaTela(final NaturezaProduto vo)  {
		throw new UnsupportedOperationException("Fazer override de transfereVoParaTela em NaturezaProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.trata_natureza_produto,"R.layout.trata_natureza_produto");
		return recurso;
	}
	...
	protected final void inicializaItensTelaBase() {
		if (item==null) {
			...
			item = FabricaVo.criaNaturezaProduto();
