...
import coletapreco.view.adapter.PalavraListAdapter;
import coletapreco.view.adapter.listaedicao.PalavraListEdicaoAdapter;
...
public abstract class PalavraListaEdicaoBase extends BaseListFragment implements IQuadroListaEdicao  
	{

	public final static String ITEM_CLICK = "PalavraItemClick";

	private PalavraListEdicaoAdapter adapt = null;
	private List<Palavra> lista = null;
	private Button btnCriaItem = null;
	
	private boolean salvaPreliminar = false;
	
	private PalavraServico servico = null;
	...
	protected PalavraServico getServico() {
		return servico;
	}
	
	@Override
	protected final void inicializaItensTelaBase() {
		btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaPalavra);
	}
	@Override
	...
	protected final void inicializaServicosBase() {
		servico = FabricaServico.getInstancia().getPalavraServico(FabricaServico.TIPO_SQLITE);
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_palavra,"R.layout.lista_palavra");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//Palavra nova = servico.inicializaItemTela(getContext());
		Palavra nova = FabricaVo.criaPalavra();
		nova = insereObjetoPrincipal(nova);
		nova = (Palavra) ((ServicoLocal)servico).atribuiUsuario(nova);
	...
	/*
	protected Palavra criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em PalavraQuadroLista retornando new Palavra com inicializa??o de listas internas se necessario");
	...
	*/
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em PalavraListaEdicao retornando new PalavraQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private void preencheLista() {
        ...
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<Palavra> -> " + lista.size() + " itens");
	...
	private PalavraListEdicaoAdapter getAdapter(List<Palavra> lista,DigicomContexto dContexto) {
		return new PalavraListEdicaoAdapter(lista,this,dContexto.getContext());
	}
	...
	private Palavra insereObjetoPrincipal(Palavra item) {
	
		return item;
	}
	
	
	protected List<Palavra> getListaCorrente(Context contexto,PalavraServico servico) {
		List<Palavra> saida = servico.getAllTela(contexto);
		return saida;
	}
