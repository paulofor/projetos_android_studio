...
import coletapreco.view.adapter.UsuarioPesquisaListAdapter;
import coletapreco.view.adapter.listaedicao.UsuarioPesquisaListEdicaoAdapter;
...
public abstract class UsuarioPesquisaListaEdicaoBase extends BaseListFragment implements IQuadroListaEdicao  
	{

	public final static String ITEM_CLICK = "UsuarioPesquisaItemClick";

	private UsuarioPesquisaListEdicaoAdapter adapt = null;
	private List<UsuarioPesquisa> lista = null;
	private Button btnCriaItem = null;
	
	private boolean salvaPreliminar = false;
	
	private UsuarioPesquisaServico servico = null;
	...
	protected UsuarioPesquisaServico getServico() {
		return servico;
	}
	
	@Override
	protected final void inicializaItensTelaBase() {
		btnCriaItem = (Button) getTela().findViewById(R.id.btnCriaUsuarioPesquisa);
	}
	@Override
	...
	protected final void inicializaServicosBase() {
		servico = FabricaServico.getInstancia().getUsuarioPesquisaServico(FabricaServico.TIPO_SQLITE);
	...
	protected ResourceObj getLayoutTela() {
		ResourceObj recurso = new ResourceObj(R.layout.lista_usuario_pesquisa,"R.layout.lista_usuario_pesquisa");
		return recurso;
	}
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//UsuarioPesquisa nova = servico.inicializaItemTela(getContext());
		UsuarioPesquisa nova = FabricaVo.criaUsuarioPesquisa();
		nova = insereObjetoPrincipal(nova);
		nova = (UsuarioPesquisa) ((ServicoLocal)servico).atribuiUsuario(nova);
	...
	/*
	protected UsuarioPesquisa criaNova() {
		return servico.inicializaItemTela(getContext());
		//throw new UnsupportedOperationException("Fazer override de criaNova em UsuarioPesquisaQuadroLista retornando new UsuarioPesquisa com inicializa??o de listas internas se necessario");
	...
	*/
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em UsuarioPesquisaListaEdicao retornando new UsuarioPesquisaQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	private void preencheLista() {
        ...
        UsuarioPesquisaServico servico = FabricaServico.getInstancia().getUsuarioPesquisaServico(FabricaServico.TIPO_SQLITE);
        DigicomContexto dContexto = getContext();
        lista = getListaCorrente(dContexto.getContext(),servico);
        DCLog.d(DCLog.SERVICO_QUADRO_LISTA, this, "preencheLista : List<UsuarioPesquisa> -> " + lista.size() + " itens");
	...
	private UsuarioPesquisaListEdicaoAdapter getAdapter(List<UsuarioPesquisa> lista,DigicomContexto dContexto) {
		return new UsuarioPesquisaListEdicaoAdapter(lista,this,dContexto.getContext());
	}
	...
	private UsuarioPesquisa insereObjetoPrincipal(UsuarioPesquisa item) {
	
		return item;
	}
	
	
	protected List<UsuarioPesquisa> getListaCorrente(Context contexto,UsuarioPesquisaServico servico) {
		List<UsuarioPesquisa> saida = servico.getAllTela(contexto);
		return saida;
	}
