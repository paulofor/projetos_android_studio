...
public abstract class PalavraProdutoQuadroListaBase extends BaseListFragment implements IQuadroListaEdicao {

	public final static String ITEM_CLICK = "PalavraProdutoItemClick";

	PalavraProdutoListAdapter adapt = null;
	private Button btnCriaItem = null;
	
	PalavraProdutoServico servico = null;
	...
	public final void chamaCriaItem() {
		IFragmentEdicao quadro = criaQuadroTrata();
		//PalavraProduto nova = servico.inicializaItemTela(getContext());
		PalavraProduto nova = FabricaVo.criaPalavraProduto();
	...
	protected IFragmentEdicao criaQuadroTrata() {
		throw new UnsupportedOperationException("Fazer override de criaQuadroTrata em PalavraProdutoQuadroLista retornando new PalavraProdutoQuadroTrata ou verificar se nao esta sendo chamada via super");
	}
	...
	protected PalavraProdutoListAdapter getAdapter(List<PalavraProduto> lista,DigicomContexto dContexto) {
		return new PalavraProdutoListAdapter(lista,dContexto.getContext());
	}
	
	protected List<PalavraProduto> getListaCorrente(Context contexto,PalavraProdutoServico servico) {
		List<PalavraProduto> saida = servico.getAllTela(contexto);
		return saida;
	}
