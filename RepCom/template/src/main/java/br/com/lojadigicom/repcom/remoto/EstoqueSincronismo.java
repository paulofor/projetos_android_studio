...
public class EstoqueSincronismo {
	...
	public void sincroniza(Context contexto, boolean forcaAtualizacao, boolean delete, boolean atualizaLocal) {
	 	EstoqueRemote servicoRemoto = new  EstoqueRemote(); // fazer estatico ?
        try {
             ContentResolver resolver = contexto.getContentResolver();
             Cursor cursor = resolver.query(EstoqueContract.buildAllSinc(), EstoqueContract.COLS_SINC, null, null, null);
             int tam = cursor.getCount();
             if (cursor.getCount() > 0 ) {
                  servicoRemoto.listaSincronizadaAlteracao(cursor, contexto);
                  DCLog.d(DCLog.SINCRONIZACAO_SINCRONIZADOR, this, "Estoque: " + tam +  " >> ");
                  resolver.delete(EstoqueContract.buildDeleteAllSinc(),null,null);
             }
