...
import br.com.lojadigicom.repcom.modelo.ParcelaVendaVo;
...
public class ParcelaVendaMontador implements MontadorDaoI {

	private ParcelaVenda principal = new ParcelaVendaVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (ParcelaVenda) entrada;
		item.setIdParcelaVenda(getInt(c,pos++));
		item.setValorParcela(getFloat(c,pos++));
		item.setDataVencimento(this.getTimestampData(c,pos++));
		item.setNumeroParcela(getInt(c,pos++));
		item.setPaga(getLogic(c,pos++));
		item.setIdVendaPa(getInt(c,pos++));
