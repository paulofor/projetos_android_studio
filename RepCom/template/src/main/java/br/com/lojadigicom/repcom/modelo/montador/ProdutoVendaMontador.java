...
import br.com.lojadigicom.repcom.modelo.ProdutoVendaVo;
...
public class ProdutoVendaMontador implements MontadorDaoI {

	private ProdutoVenda principal = new ProdutoVendaVo();
	
	private boolean sinc = false;
	public ProdutoVendaMontador(boolean sinc) {
		this.sinc = sinc;
	}
	public ProdutoVendaMontador() {
		this.sinc = false;
	}
	...
	public DCIObjetoDominio getItem(Cursor c) {
		...
		//objeto = FabricaVo.criaProdutoVenda();
		objeto = new ProdutoVendaVo();
	...
	public DCIObjetoDominio getItem(Cursor c, DCIObjetoDominio entrada, int pos) {
		...
		item = (ProdutoVenda) entrada;
		item.setIdProdutoVenda(getInt(c,pos++));
		item.setQuantidade(getInt(c,pos++));
		item.setValorTotal(getFloat(c,pos++));
		item.setValorItem(getFloat(c,pos++));
		item.setIdProdutoA(getInt(c,pos++));
		item.setIdVendaA(getInt(c,pos++));
